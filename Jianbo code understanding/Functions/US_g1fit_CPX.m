%% fit g1, complex
function [Ms, Mf, Vx, Vy, Vz,R,GGf]=US_g1fit_CPX(GG, A_FWHM, rFrame)
% GG(nz,nx,nTau)
...
for iz=1:nz
    for ix=1:nx
        ...
        if abs(gg0(1))>0.5
            ...
            vz0=Vz0(iz,ix);
            vz0=sign(vz0)*min(abs(vz0),30e-3);
            %% 2.2. Vt0 initial
            vxmx0=[0:0.5:30]*1e-3;
            vy0=0;
            clear Rvt0;
...
%             grid on
            % determine vx0 -0
            vx0=min(min(vxmx0(MInd(1)),max((DR/0.02)^1.5*abs(vz0),3e-3)),30e-3*mfR0);
            %% fit imaginary g1
            if  max(abs(imag(gg(1:floor(10e-3*rFrame)))))>0.1
                ...
                Fmin_cstrn(:,1)=[mfI0-0.05 min((mfI0+1),1)];   % Mf constrain
                Fmin_cstrn(:,2)=(vx0+[-1*min((2*(0.9/Rvt0(MInd))^4),vx0) min(2*(0.9/Rvt0(MInd))^4,5)]*1e-3)*tauI(end)/(Sigma2(1));  % Vx constrain
...
%                 Fmin_cstrn(:,3)=(vx0+[-1*min((2*(0.9/Rvt0(MInd))^4),vx0) min(2*(0.9/Rvt0(MInd))^4,5)]*1e-3)*tauI(end)/(Sigma2(2));  % Vy constrain
                ...
                fitC0 = [mfI0, vx0*tauI(end)/(Sigma2(1)), vy0*tauI(end)/(Sigma2(2)), vz0];
                ...
                %% %%%%%%%%%%%
                ggFitI=mfI*exp(-vxI^2*tI.^2/(Sigma2(1))^2-vyI^2*tI.^2/(Sigma2(2))^2-vzI^2*tI.^2/(Sigma2(3))^2).*sin(2*k0*vzI*tI);
                RI = 1 - sum( abs(ggI-ggFitI).^2 ) / sum( abs(ggI-mean(ggI)).^2);
...
%                 end
                ...
                Fmin_cstrn(:,2)=[max(mfR0-0.05,0) min(mfR0+0.05,1)];   % MfR constrain
                Fmin_cstrn(:,3)=[max(mfI0-0.05,0) min(mfI0+0.05,1)];   % MfI constrain
                Fmin_cstrn(:,4)=(vx0+[-15*min(max(0.1*(0.9/R1)^4,0),0.5),2*min(max(0.1*(0.9/R1)^4,0),0.5)]*vx0)*tau0(end)/(Sigma2(1));  % Vx constrain
...
%                 Fmin_cstrn(:,5)=(vx0+[-15*min(max(0.1*(0.9/R1)^4,0),0.5),2*min(max(0.1*(0.9/R1)^4,0),0.5)]*vx0)*tau0(end)/(Sigma2(2));  % Vy constrain
                Fmin_cstrn(:,6)=vz0+[-2 2]*1e-3;  % Vz constrain
            else
                mfI0=max(abs(imag(gg0)));
...
%                 vy0=0;
                ...
                Fmin_cstrn(:,4)=(vx0+[-15*min(max(0.5*(0.9/R1)^4,0),5),2*min(max(0.5*(0.9/R1)^4,0),5)]*1e-3)*tau0(end)/(Sigma2(1));  % Vx constrain
                Fmin_cstrn(:,5)=[0 0]*1e-3*tau0(end)/(Sigma2(2));  % Vy constrain
%                 Fmin_cstrn(:,5)=(vx0+[-15*min(max(0.5*(0.9/R1)^4,0),5),2*min(max(0.5*(0.9/R1)^4,0),5)]*1e-3)*tau0(end)/(Sigma2(2));  % Vy constrain
