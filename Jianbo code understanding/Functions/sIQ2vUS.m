%% US g1 fit, single flow direction model
% input: 
...
function [Ms, Mf, Vx, Vz, V, pVz ,R, Vcz, GGf]=sIQ2vUS(sIQ, PRSSinfo)
%% O. constant
...
rGG=zeros(nz0*PRSSinfo.rfnScale,nx0*PRSSinfo.rfnScale,nTau);
if PRSSinfo.rfnScale>1
    for it=1:nTau
        GG(:,:,it)=imresize(GG0(:,:,it),[nz0,nx0]*PRSSinfo.rfnScale,'bilinear'); % spatial interpolation
    end
    SNR=imresize((SNR0>mean(SNR0(:))+1.5*std(SNR0(:))),[nz0,nx0]*PRSSinfo.rfnScale,'bilinear'); % spatial interpolation
else
    GG=GG0;
...
clear GG0 SNR0;
CR=(abs(GG(:,:,1))>0.2).*(SNR);
Vcz=(imresize(Vcz0,[nz0,nx0]*PRSSinfo.rfnScale,'bilinear').*CR); % color Doppler, all frequency
%% III. GG2Vz
[nz,nx,nTau]=size(GG);
...
R=((reshape((1-sum(abs(GG2-GGf0).^2,2)./sum(abs((GG2)-mean(GG2,2)).^2,2)),[nz,nx]).*CR));  
