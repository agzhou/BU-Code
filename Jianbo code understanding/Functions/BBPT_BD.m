%% microbubble pair and track - based on iBB closest, iBBest closest, upper and lower distance range criteria 
...
%     CoorBB{it}=[zBB,xBB];
% end
function [BBPD]=BBPT_BD(CoorBB,PRSSinfo) % use the coordinates of BB
nt=length(CoorBB);
    ...
    for iBB=1:nBB
        ...
        for iTrkF=1:PRSSinfo.nTrack % try to pair and track the same bubble in the next PRSSinfo.nTrack frames
            if iTrkF==1
                ...
                dCrit=(PRSSinfo.dCrit/PRSSinfo.lPix); %[lower bound, upper bound], distance range between BB(t+1) and BB(t) 
                dEstCrit=(PRSSinfo.dCrit(2)/PRSSinfo.lPix); % maximum distance between BBest(t+1) and BB(t+1) 
                mV=0;
            else
                % [lower bound, upper bound], distance range between BB(t+1) and BB(t)
                dCrit=[(PRSSinfo.dCrit(1)/PRSSinfo.lPix),max(min((mV)*2,8),(PRSSinfo.dCrit(2)/PRSSinfo.lPix))];
...
%             hold on,plot(iBBCoorCBB(2),iBBCoorCBB(1),'r<'); hold on,plot(estCoorNPD(2),estCoorNPD(1),'bo');
            ...
            indCrit=find((dBBnext>=dCrit(1)).*(dBBnext<=dCrit(2)));
            if ~isempty(indCrit)
                ...
                if ~isempty(dBBnext)
                    ...
                    if dnbBBnext>=dCrit(1) && dnbBBnext<=dCrit(2)
