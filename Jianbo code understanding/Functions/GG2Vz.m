...
%% III. Vz
Vz=(((lambda./(2*Tvz).*sign(mean(iGG(1:3,:),1))).')); % absolute value of axial velocity, m/s
% Vz=reshape(medfilt2(reshape(Vz0,[PRSSinfo.Dim(1),PRSSinfo.Dim(2)]),[3,3]),[PRSSinfo.Dim(1)*PRSSinfo.Dim(2),1]).*CR;
