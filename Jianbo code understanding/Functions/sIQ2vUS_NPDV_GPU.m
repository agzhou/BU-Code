...
function [Mf, Vz, V, pVz, Vcz, R, CR, Vx, Ms, pnRatio,GGf,sumGG0,sumGGV0,sumGGFV0]=sIQ2vUS_NPDV_GPU(sIQ, PRSSinfo)
%% O. constant
...
%     fCR=fCR+(1-fCR).*fSNR0*0.5;
    ...
    iCR=gather(((pnCR+fCR0)>0).*((fCR+ggCR0)>0).*ggCR.*aCR);  % acceptable signal criteria
%     iCR=gather(((((pnCR+fCR0)>0).*fCR.*ggCR.*aCR)+mean(abs(iGG(:,:,1:3)),3)>0.4)>0);  % acceptable signal criteria
    %% IV Color Doppler
    iVcz=gather(ColorDoppler_NP(iIQ,PRSSinfo)); % color Doppler
    ...
    ggCR0=((ggCR1+ggCR2)>0).*(1-(real(GG2(:,1))>0.7)); % modified by Bingxue Liu;
...
%     [iVz, iVx, ipVz, iMs, iMf, iR, iGGf]=GG2vUS(GG2, reshape(iVcz,[nz0*nx0,1]), Ms00, MfR0, PRSSinfo);
    Ms0(:,:,iNP)=gather(iMs); Mf0(:,:,iNP)=gather(iMf(:,:,1));
    Vx0(:,:,iNP)=gather(iVx).*iCR; Vz0(:,:,iNP)=gather(iVz).*iCR; Vcz0(:,:,iNP)=-1*iVcz.*iCR;
    pVz0(:,:,iNP)=gather(ipVz).*iCR; R0(:,:,iNP)=gather(iR); GGf0(:,:,:,iNP)=gather(iGGf);
    CR0(:,:,iNP)=gather(iCR);
    pnRatio0(:,:,iNP)=gather(ipnRatio);
...
%     hold on; plot((imag(squeeze(iGG__(slt(1),slt(2),:,1)))));
    sumGG0(:,iNP) = squeeze(sqrt(abs(-log((GG2_(:,5))./(GG2_(:,1))))));
    sumGGV0(:,iNP) = squeeze((GG2_(:,1))./(1-(GG2_(:,1))));
    sumGGFV0(:,iNP) = squeeze((real(GG2(:,1))-(real(GG2(:,10)))));
