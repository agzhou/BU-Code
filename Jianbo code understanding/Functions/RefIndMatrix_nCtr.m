...
function [IndCtriMatrix,ApodChn]=RefIndMatrix_nCtr(P,xCoor,zCoor, nElem)
tic;
if nargin<4
    nElem=2;
end
...
iP.pitch=P.pitch; % pitch between transducer elements, mm
iP.Wavelength=P.vSound*1e3/(P.frequency*1e6); % mm
iP.dSample=iP.Wavelength/P.nSmplPerWvlnth; % distance between samplling points, mm, the default is 4 sampling points per wavelength
iP.nZsample= P.actZsamples; % number of samples for each pulse-echo acquisition
...
for iAgl=1:P.numAngles
    ...
    for ix=1:nx            % main loop, coordinates of the x points
        for iz=1:nz
            [IndCtriMatrix(iz,ix,:,iAgl),ApodChn(iz,ix,:,iAgl)]=CtribtPixel_nCtr(iP,nElem, xCoor(ix),zCoor(iz));      % calculate delays and apodization
        end%
    end
end
toc