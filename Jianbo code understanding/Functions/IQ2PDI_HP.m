...
function PDIHP=IQ2PDI_HP(IQ,PRMT)
% IQ: IQ data
...
dz=PRMT.dz;             % axial samppling step, in mm, the default is 4 samppling points for one wavelength  
%% x,z coordinates after image refined
[nz,nx,nt]=size(IQ);
xCoor=linspace(0,nx*dx,nx);
zCoor=linspace(0,nz*dz,nz);
...
% PDIHPdb=log10(PDIHP);% High pass filter-based PD image, log
