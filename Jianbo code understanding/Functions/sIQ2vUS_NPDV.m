...
function [Mf, Vz, V, pVz, Vcz, R, CR, Vx, Ms, pnRatio,GGf,sumGG0,sumGGV0,sumGGFV0,sumGG0_,GG]=sIQ2vUS_NPDV(sIQ, PRSSinfo)
%% O. constant
...
fIQ=sysNoiseRemove(sIQ,PRSSinfo.rFrame);% iNP=1,positive freq; iNP=2, negative freq
...
%     MfR0 = max(min(1-Ms00-Me0,real(GG2(:,1))),0);
    ...
    [iVz, iVx, ipVz, iMs, iMf, iR, iGGf]=GG2vUS_fit(GG2, reshape(g1Vz0,[nz0*nx0,1]), Ms00, MfR0, PRSSinfo);
...
%     hold on; plot((imag(squeeze(iGG__(slt(1),slt(2),:,1)))));
    sumGG00(:,iNP) = squeeze(sqrt(abs(-log((GG2_(:,5))./(GG2_(:,2))))));
    sumGG00_(:,iNP) = squeeze(sqrt(abs(-log((GG2_(:,5)-Ms00)./(GG2_(:,2)-Ms00)))));
    ...
    sumGG00_(:,3) = squeeze(sqrt(abs(-log(abs(GG0(:,5))./abs(GG0(:,1))))));
...
sumGG0_ = reshape(sumGG00_, [PRSSinfo.Dim(1),PRSSinfo.Dim(2), 3]);
