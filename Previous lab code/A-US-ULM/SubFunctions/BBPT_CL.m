...
%     CoorBB{it}=[zBB,xBB];
% end
function [BBPD]=BBPT_CL(CoorBB,PRSSinfo) % use the coordinates of BB
nt=length(CoorBB);
...
%             hold on,plot(iBBCoorCBB(2),iBBCoorCBB(1),'r<'); hold on,plot(estCoorNPD(2),estCoorNPD(1),'bo');
            ...
            sPositionInd=find(dBBnext==0);  % The same position BB
            ...
            CoorNBB(sPositionInd,:)=[];
             if ~isempty(dBBnext)
                ...
                if dnbBBnext<dCrit && (MutualMin==0)  % mutual closest
