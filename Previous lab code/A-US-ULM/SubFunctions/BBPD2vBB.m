...
function [BB,BBV, BBVz]=BBPD2vBB(BBPD,PRSSinfo)
% input: track and paired BB info
nRfn=PRSSinfo.lPix/PRSSinfo.IntPixSize;
PRSSinfo.Dim=PRSSinfo.Dim*nRfn;
    ...
    for iBB=1:inPDBB
        if ~isempty (BBPD{it})
            ...
            if nTrk>=PRSSinfo.thdTrk % Trackable threshold
                ...
                for iTrk=1:nTrk
%                     iv=v0(iTrk);
%                     if abs(iv-mV(iTrk))>10
%                         continue;
%                     end
                    if iBBPD(1,iTrk)~=0
                        % Path interpolation
                        lZ=ceil(abs(iBBPD(1,iTrk)-iBBPD(3,iTrk)))*nRfn;
                        lX=ceil(abs(iBBPD(2,iTrk)-iBBPD(4,iTrk)))*nRfn;
                        nPixInt=max(lZ,lX);
                        intZ=round(linspace(iBBPD(1,iTrk),iBBPD(3,iTrk),nPixInt)*nRfn);
                        intX=round(linspace(iBBPD(2,iTrk),iBBPD(4,iTrk),nPixInt)*nRfn);
                        ...
                        for iC=1:numel(intZ)
                            ...
                            if BBV(intZ(iC),intX(iC),1)==0
                                BBVz(intZ(iC),intX(iC),vDir)=abs(intVz(iC))*sign(vDir-1.5);
                                BBV(intZ(iC),intX(iC),vDir)=intV(iC)*sign(vDir-1.5);
                            else
                                BBVz(intZ(iC),intX(iC),vDir)=(BBVz(intZ(iC),intX(iC),vDir)+abs(intVz(iC))*sign(vDir-1.5))/2;
                                BBV(intZ(iC),intX(iC),vDir)=(BBV(intZ(iC),intX(iC),vDir)+intV(iC)*sign(vDir-1.5))/2;
                            end
                        end
                    end
                end
