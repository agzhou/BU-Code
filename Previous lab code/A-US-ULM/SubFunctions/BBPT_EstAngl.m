%% microbubble pair and track - based on iBBest closest and angle criteria 
...
%     CoorBB{it}=[zBB,xBB];
% end
function [BBPD]=BBPT_EstAngl(CoorBB,PRSSinfo) % use the coordinates of BB
nt=length(CoorBB);
    ...
    for iBB=1:nBB
        ...
        for iTrkF=1:PRSSinfo.nTrack % try to pair and track the same bubble in the next PRSSinfo.nTrack frames
            if iTrkF==1
                ...
                dCrit=max(min((mV)*2,6),12);% maximum distance between BB(t+1) and BB(t) 
                dEstCrit=min(max(mV*1,4),8); % maximum distance between BBest(t+1) and BB(t+1) 
            end
            %% pairing frames
...
%             hold on,plot(iBBCoorCBB(2),iBBCoorCBB(1),'r<'); hold on,plot(estCoorNPD(2),estCoorNPD(1),'bo');
            ...
            if min(dBBnext)>dCrit
                continue;
            end
            %% BBs in the it+iTrkF frame within the distance range of iBB in it+iTrkF-1 frame
            indCrit=find(dBBnext<dCrit);
            BBCoorInCrit=CoorNBB(indCrit,:); % BBs within the distance range dCrit of iBBCoorCBB
            %% BBs in the it+iTrkF frame also within the estimated iBB
            dEstBB=sqrt(sum(((estCoorNPD-BBCoorInCrit).^2),2)); % distance between the estimated iBB and other BB in frame it+iTrkF
            indCritEst=find(dEstBB<dEstCrit);
            BBCoorInCrit=BBCoorInCrit(indCritEst,:); % BBs within the mutual distance range of iBBCoorCBB (dCrit) and estCoorNPD(dEstCrit)
            %% the closest BB to iBB in it+iTrkF-1 frame
            dBBnext=sqrt(sum(((iBBCoorCBB-BBCoorInCrit).^2),2)); % distance between the iBB and other BBs within the critera range in frame it+iTrkF
            
            if ~isempty(dBBnext)
                dnbBBnext=min(dBBnext); % distance between the closest BB in next frame
                indPD=find(dBBnext==dnbBBnext);
                iBBCoorNPD=BBCoorInCrit(indPD(1),:); % the forward paired BB coordinate in the it+iTrkF frame
                %% backward pairing
                ...
                if (MutualMin==0)  % mutual closest
                    v=sqrt(sum(iBBCoorNPD-iBBCoorCBB).^2)*PRSSinfo.lPix/1e3*PRSSinfo.CCFR/(nMis+1);
