...
%%%%%%%%% Data is stored as [z, x, time, coronal plane] %%%%%%%%%
indSkipped=1; k = 1; 
...
PDI = PDIHP0./eqNoise.^1.8; %%%%%%%%% Gain compensation for PDI %%%%%%%%%
ImgBG = mean(squeeze(PDI(:,:,3,:)),3).^0.35; %%%%%%%%% trial average the PDI %%%%%%%%%
[nz,nx,ntau,~] = size(GG0);
...
absGG = abs(GG0);
phaseGG = angle(GG0);
absnGG = absGG./absGG(:,:,1,:); %%%%%%%%% normalized |g1| %%%%%%%%%
rnGG = rGG./rGG(:,:,1,:);       %%%%%%%%% relative |g1| %%%%%%%%%

% absGGHP = medfilt1(absGG, 3, [], 3);
absGGHP = movmean(absGG, 3, 3); %%%%%%%%% This is low pass, not high pass lmfao %%%%%%%%%

mphaseGG = mean(angle(GG0),4); %%%%%%%%% mean phase of the |g1| across coronal planes?? %%%%%%%%%
...
dir = sum(mphaseGG(:,:,1:10),3); %%%%%%%%% axial flow direction for each pixel using the first 10 tau points %%%%%%%%%
dir(dir>0) = 1;  %%%%%%%%% binarize the positive directions %%%%%%%%%
dir(dir<0) = -1; %%%%%%%%% binarize the negative directions %%%%%%%%%
mphaseGG_bar = mphaseGG.*dir; %%%%%%%%% turn the negative parts of mphaseGG to positive? %%%%%%%%%
