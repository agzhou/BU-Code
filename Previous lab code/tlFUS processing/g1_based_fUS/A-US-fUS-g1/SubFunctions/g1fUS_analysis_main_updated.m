function [ROIGG, ROIGGV, ROIGGF, ROIGGFV, ROIPDI, ROIVV, ROIVVcz, Ratio, Peak, T2Peak, CNR, anpratio, actmap, ImgBG] = g1fUS_analysis_main_updated
% clear all; close all;
addpath D:\g1_based_fUS\A-US-fUS-g1\SubFunctions\

%% I.load GG data
[FileName,FilePath]=uigetfile('V:\G1based fUS data\vUS\');
...
%     GG = myFile.GG;
    Ms = myFile.Ms;
    Mf = myFile.Mf;
    sumGG0 = myFile.sumGG0;% -Ms 
    sumGGV0 = myFile.sumGGV0;
    %sumGGV0 = squeeze((Mf(:,:,:,1))./(1-(Mf(:,:,:,1))));
...
sumGGV(:,:,:,k) = sumGGV0;
sumGGFV(:,:,:,k) = sumGGFV0;
sumGGF(:,:,:,k) = sumGGV0(:,:,1:2).*sumGG0;%VV;%sumGGV0.*;% PDI0(:,:,1:2).*VVcz;
...
% %%
% sumGG0 = squeeze(sqrt(abs(-log((GG0_(:,5,:,:))./(GG0_(:,2,:,:)))))); % (5,1);(5,2);(5,3);(5,4);(6,2);%(7,2);
...
trial.stim = stim;
%% image background
ImgBG = median(squeeze(PDI(:,:,3,:)),3).^0.35;
...
%               figure; imagesc(actmap_XX(:,:,iNP)); axis image;colormap(jet);caxis([0,1]);
%               [loc_x,loc_y]=ginput(1);
%               loc_x = round(loc_x);loc_y = round(loc_y);
%               MSKXX = zeros(nz,nx);
%               MSKXX(loc_y-5:loc_y+5,loc_x-5:loc_x+5) = 1; % square box ROI
        end
        BW(:,:,iNP) = MSKXX.*(actmap_XX(:,:,iNP)>0);
        %count a/v pixels in actmap
        Npixel(:,iNP) = sum(BW(:,:,iNP).*(coefmap_XX(:,:,iNP)>0),'all'); 
    end
    switch datatype
        case 'G1(rCBFv)'
            actmap.GG = actmap_XX; amp.GG = ampXX0; CNR.GG = cnrXX_final;BWs.GG = BW;anpratio.GG = Npixel; 
        case 'G1(rCBV)'
            actmap.GGV = actmap_XX; amp.GGV = ampXX0; CNR.GGV = cnrXX_final;BWs.GGV = BW;anpratio.GGV = Npixel; 
        case 'G1(rCBF)'
            actmap.GGF = actmap_XX; amp.GGF = ampXX0; CNR.GGF = cnrXX_final;BWs.GGF = BW;anpratio.GGF = Npixel; 
        case 'G1(optimal)'
            actmap.GGFV = actmap_XX; amp.GGFV = ampXX0; CNR.GGFV = cnrXX_final;BWs.GGFV = BW;anpratio.GGFV = Npixel; 
        case 'PDI'
            actmap.PDI = actmap_XX; amp.PDI= ampXX0; CNR.PDI= cnrXX_final;BWs.PDI = BW;anpratio.PDI = Npixel; 
        case 'G1(vUS)'
            actmap.VV = actmap_XX; amp.VV= ampXX0; CNR.VV= cnrXX_final;BWs.VV = BW;anpratio.VV = Npixel; 
        case 'ColorDoppler'
            actmap.VVcz = actmap_XX; amp.VVcz= ampXX0; CNR.VVcz = cnrXX_final;BWs.VVcz = BW;anpratio.VVcz = Npixel; 
    end  
end

clear trialXX
%% data analysis % calculate roi time course in same roi
libmsk = {'MSKGG', 'MSKGGV', 'MSKGGF'};
for k = 1:3
    MSKtype = libmsk{k};
switch MSKtype
    case 'MSKGG'
...
for i = 1:numel(lib)
    ...
    switch datatype
        case 'G1(rCBFv)'
            XX = sumGG; Npixel = anpratio.GG;
        case 'G1(rCBV)'
            XX = sumGGV; Npixel = anpratio.GGV;
        case 'G1(rCBF)'
            XX = sumGGF; Npixel = anpratio.GGF;
        case 'G1(optimal)'
            XX = sumGGFV; Npixel = anpratio.GGFV;
        case 'PDI'
            XX = PDI; Npixel = anpratio.PDI;
        case 'G1(vUS)'
            XX = VV0; Npixel = anpratio.VV;
        case 'ColorDoppler'
            XX = VVcz0; Npixel = anpratio.VVcz;
    end
    %normalize data and preprocess
    trialXX = reshape(XX(:,:,1:2,trial.nBase-5+1: end-5), [nz,nx,2,trial.nlength,trial.n]);
    mtrialXX = median(trialXX,5);
    ...
    for iNP = 1:2
        ...
        roiXX(iNP,:) = squeeze(sum(sum(squeeze(XX(:,:,iNP,:)).*repmat(BWk(:,:,iNP).*vmsk,[1, 1,nRpt]),1),2))/(Npixel(:,iNP)+1e-6);
    end
    roiXXall = abs(roiXX(1,:)).*Npixel(:,1)/sum(Npixel,2)+roiXX(2,:).*Npixel(:,2)/sum(Npixel,2);
    ROIXX{k} = averagedTrials(roiXXall, trial);
    ROIXX{k}.vein = averagedTrials(roiXX(1,:), trial);
    ROIXX{k}.artery = averagedTrials(roiXX(2,:), trial);
    [vpXX,~] = max(movmean(ROIXX{k}.vein.m(window(1):window(2)), 3)/100);
    [apXX,~] = max(movmean(ROIXX{k}.artery.m(window(1):window(2)), 3)/100);
    vtpXX = calTime2Peak(ROIXX{k}.vein.m, trial);
    if isnan(ROIXX{k}.artery.m)==1
        atpXX = vtpXX;
    else
    atpXX = calTime2Peak(ROIXX{k}.artery.m, trial);
    end
    switch datatype
        case 'G1(rCBFv)'
            ROIGG{k} = ROIXX{k};Peak{k}.GG.vein = vpXX;Peak{k}.GG.artery = apXX;T2Peak{k}.GG.vein = vtpXX;T2Peak{k}.GG.artery = atpXX;
        case 'G1(rCBV)'
            ROIGGV{k} = ROIXX{k};Peak{k}.GGV.vein = vpXX;Peak{k}.GGV.artery = apXX;T2Peak{k}.GGV.vein = vtpXX;T2Peak{k}.GGV.artery = atpXX;
        case 'G1(rCBF)'
            ROIGGF{k} = ROIXX{k};Peak{k}.GGF.vein = vpXX;Peak{k}.GGF.artery = apXX;T2Peak{k}.GGF.vein = vtpXX;T2Peak{k}.GGF.artery = atpXX;
        case 'G1(optimal)'
            ROIGGFV{k} = ROIXX{k};Peak{k}.GGFV.vein = vpXX;Peak{k}.GGFV.artery = apXX;T2Peak{k}.GGFV.vein = vtpXX;T2Peak{k}.GGFV.artery = atpXX;
        case 'PDI'
            ROIPDI{k} = ROIXX{k};Peak{k}.PDI.vein = vpXX;Peak{k}.PDI.artery = apXX;T2Peak{k}.PDI.vein = vtpXX;T2Peak{k}.PDI.artery = atpXX;
        case 'G1(vUS)'
            ROIVV{k} = ROIXX{k};Peak{k}.VV.vein = vpXX;Peak{k}.VV.artery = apXX;T2Peak{k}.VV.vein = vtpXX;T2Peak{k}.VV.artery = atpXX;
        case 'ColorDoppler'
            ROIVVcz{k} = ROIXX{k};Peak{k}.VVcz.vein = vpXX;Peak{k}.VVcz.artery = apXX;T2Peak{k}.VVcz.vein = vtpXX;T2Peak{k}.VVcz.artery = atpXX;
    end  
end
Ratio{k}.all = (max(movmean(ROIGGF{k}.m, 3)/100)-1)/(max(movmean(ROIPDI{k}.m, 3)/100)-1);
Ratio{k}.vein = (max(movmean(ROIGGF{k}.vein.m, 3)/100)-1)/(max(movmean(ROIPDI{k}.vein.m, 3)/100)-1); 
Ratio{k}.artery = (max(movmean(ROIGGF{k}.artery.m, 3)/100)-1)/(max(movmean(ROIPDI{k}.artery.m, 3)/100)-1); 
end
save([FilePath, 'ROIdata.mat'],'ROIGG', 'ROIGGV', 'ROIGGF', 'ROIGGFV', 'ROIPDI','ROIVV','ROIVVcz');
%%
Fig = figure;
...
ampGG2 = reshape(amp.GG,[nx*nz,2]); ampGGV2 = reshape(amp.GGV,[nx*nz,2]);
BWall = (ampGG2(:,1)<2).*(ampGG2(:,2)<2).*(ampGGV2(:,1)<2).*(ampGGV2(:,2)<2).*BWall(:);
