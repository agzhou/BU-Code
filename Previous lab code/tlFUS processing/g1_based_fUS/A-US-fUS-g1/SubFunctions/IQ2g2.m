...
function [GG, Numer] = IQ2g2(IQ, Start_T, nt, ntau)
Intensity = sqrt(IQ.*conj(IQ));
...
% end
% Denom=repmat(mean(temp_deno,3),[1,1,ntau]); % calculate the denomenator
% Numer=(mean(temp_numer,4));
% GG = Numer./Denom;
%% calculate g2 pixel by pixel
GG=zeros(nz,nx,ntau);
for iz=1:nz
    ...
    temp_deno = Intensity;
    for itau = 1:ntau
        temp_numer(1,:,itau,:)=((Intensity(iz,:,Start_T:Start_T-1+nt)).*(Intensity(iz,:,itau+Start_T:itau+Start_T-1+nt)));
    end
    Denom=mean(temp_deno,3).^2; % calculate the denomenator
    Numer=(mean(temp_numer,4));
    GG(iz,:,:) = Numer./Denom;
