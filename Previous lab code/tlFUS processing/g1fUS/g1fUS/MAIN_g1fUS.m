...
clear all; clc
defaultpath='F:\BU-Data\Stroke7-20181113-Acute\vUS-Baseline';
...
covB=ones(3,3); covB(3,3)=7; covB=covB/sum(covB(:));
%% data processing
...
%             PDIHHP(:,:,iD)=imresize(PDIHHP0(:,:,iD),[nz,nx]*PRSSinfo.rfnScale,'bilinear');
        ...
        if PRSSinfo.inVivo==1
            ...
            [nz,nx,nt]=size(sIQ);
            npGG=zeros(nz,nx,PRSSinfo.g1nTau);
            unNorm_npGG=npGG;
            Vcz=zeros(nz*PRSSinfo.rfnScale,nx*PRSSinfo.rfnScale,3);
            ...
            for iNP=1:2
                ...
                npGG=sIQ2GG_GPU(iIQ, PRSSinfo); % g1 of p or n frequency signal    
                % g1(1) adjustment
...
%                 CBF_speedInd(:,:,iNP) = abs(squeeze(sqrt(log(abs(GGsmth(:,:,Tau1))./abs((GGsmth(:,:,Tau2)))))));
                CBF_speedInd(:,:,iNP) = abs(squeeze(sqrt(abs(log(GGsmth(:,:,Tau1)./(GGsmth(:,:,Tau2))))))).^2;
                CBVind(:,:,iNP) = squeeze(mean(abs(GGsmth(:,:,1)),3)./(1-mean(abs(GGsmth(:,:,1)),3)));
                
                %% no normalized g1 calculation
                unNorm_npGG=((sIQ2GG_unNorm_GPU(iIQ, PRSSinfo))).^0.5; % g1 of p or n frequency signal                
                % g1(1) adjustment
...
%                 unNorm_npGG=unNorm_npGG-prctile(abs(unNorm_npGG(:)),0.2);
                ...
                unNorm_npGG=unNorm_npGG-min((unNorm_npGG(:)));
                unNorm_npGG=unNorm_npGG/(max((unNorm_npGG(:))));
...
%                 end                                
%                 unNorm_CBF_speedInd(:,:,iNP) = abs(squeeze(sqrt(abs(log(unNorm_npGG(:,:,Tau1))./unNorm_npGG(:,:,Tau2)))));
                unNorm_CBF_speedInd(:,:,iNP) = abs(squeeze(sqrt((unNorm_npGG(:,:,Tau1))-(unNorm_npGG(:,:,Tau2)))));
...
%             CBF_speedInd(:,:,3) = abs(squeeze(sqrt(log(abs(GG(:,:,Tau1))./abs(GG(:,:,Tau2))))));
            CBF_speedInd(:,:,3) = abs(squeeze(sqrt(abs(log(GG(:,:,Tau1)./GG(:,:,Tau2)))))).^2;
            CBVind(:,:,3) = squeeze(mean(abs(GG(:,:,1)),3)./(1-mean(abs(GG(:,:,1)),3)));
            
            unNorm_npGG=((sIQ2GG_unNorm_GPU(sIQ, PRSSinfo))).^0.5; % g1 of p or n frequency signal
            for itau=1:PRSSinfo.g1nTau
                unNorm_npGG(:,:,itau)=convn(unNorm_npGG(:,:,itau),covB,'same');
            end
...
%             end            
            unNorm_npGG = smoothdata(unNorm_npGG, 3, 'sgolay', 9);
            unNorm_CBF_speedInd(:,:,3) = abs(squeeze(sqrt((unNorm_npGG(:,:,Tau1))-(unNorm_npGG(:,:,Tau2)))));
            ...
            disp('sIQ to GG to CBF indices...');
            sIQn=sIQ+2e6*(rand(size(sIQ))-0.5); % add noise to signal for CBV
            GG = sIQ2GG_GPU(sIQn, PRSSinfo);
            for itau=1:PRSSinfo.g1nTau
                GG(:,:,itau)=convn(GG(:,:,itau),covB,'same');
            end
            GG = smoothdata(GG, 3, 'sgolay', 9);
%             CBF_speedInd = abs(squeeze(sqrt(log(abs(GG(:,:,Tau1))./abs(GG(:,:,Tau2)))))/(sqrt(Tau2^2-Tau1^2)*0.1));
            CBF_speedInd = abs(squeeze(sqrt(abs(log(GG(:,:,Tau1))./GG(:,:,Tau2)))))/(sqrt(Tau2^2-Tau1^2)*0.1);
            CBVind(:,:) = squeeze(mean(abs(GG(:,:,1)),3)./(1-mean(abs(GG(:,:,1)),3)));            
           
            disp('CBF and CBV indices calculated ...');
            save([FilePath, 'g1fUSaddNoise',iFileName(3:end-4),'.mat'],'-v7.3','CBF_speedInd','CBVind', 'PDI', 'Vcz','P');            
...
%             caxis(speedAxisRange)
            ...
            saveas(Fig,[FilePath, 'g1fUS',iFileName(3:end-4),'.tif'],'tif');
            saveas(Fig,[FilePath, 'g1fUS',iFileName(3:end-4),'.fig'],'fig');
            close(Fig)
        end
        ...
        if PRSSinfo.inVivo==1
            %% plot the normalized g1-based CBF indices
            [VzCmap]=Colormaps_fUS;
            CBVeqNoise=(abs(CBVind).*eqNoise.^2).^0.3;
            PDIeqNoise=log(abs(PDI)./eqNoise.^2);
            %         speedAxisRange=[prctile(CBF_speedInd(:),1) prctile(CBF_speedInd(:),99.5)];
            %         CBVAxisRange=[prctile(CBVind(:),12) prctile(CBVind(:),99)];
%             speedAxisRange=[0.05 1.5];
            speedAxisRange=[-2 2];
            CBVAxisRange=[0.8 4];
            PDIaxisRange=[32 38];
            Fig=figure('visible','on');
            ...
            colormap(ax(1),VzCmap)
            caxis(speedAxisRange)
            title('CBF-speed Index, negFreq')
            
            ax(5)=subplot(3,4,5);
            imagesc(-1*CBF_speedInd(:,:,2));
            colormap(ax(5),VzCmap)
            caxis(speedAxisRange)
            ...
            ax(6)=subplot(3,4,6);
            imagesc((Vcz(:,:,2)));
            colormap(ax(6),VzCmap)
            caxis([-15 15])
            title('Vcz Index, posFreq')
            
            ax(10)=subplot(3,4,10);
            imagesc((Vcz(:,:,3)));
            colormap(ax(10),VzCmap)
            caxis([-15 15])
            title('Vcz Index, allFreq')
            ...
            ax(4)=subplot(3,4,4);
            imagesc(PDIeqNoise(:,:,1));
            caxis(PDIaxisRange)
            colormap(ax(4),gray)
            title('PDI,negFreq')
            
            ax(8)=subplot(3,4,8);
            imagesc(PDIeqNoise(:,:,2));
            caxis(PDIaxisRange)
            colormap(ax(8),gray)
            title('PDI,posFreq')
            
            ax(12)=subplot(3,4,12);
            imagesc(PDIeqNoise(:,:,3));
            caxis(PDIaxisRange)
            colormap(ax(12),gray)
            title('PDI,allFreq')
            linkaxes(ax)
...
%             unNorm_CBVeqNoise=abs(unNorm_CBVind.*eqNoise.^0.4).^0.6;
%             PDIeqNoise=log(abs(PDI)./eqNoise.^2);
            unNorm_CBVeqNoise=(abs(unNorm_CBVind));
            unNorm_CBVeqNoise=unNorm_CBVeqNoise-min(unNorm_CBVeqNoise(:));unNorm_CBVeqNoise=unNorm_CBVeqNoise/prctile(unNorm_CBVeqNoise(:),99.97);
            unNorm_CBVeqNoise=log(unNorm_CBVeqNoise);
            PDIeqNoise=(abs(PDI));PDIeqNoise=PDIeqNoise-min(PDIeqNoise(:));PDIeqNoise=PDIeqNoise/max(PDIeqNoise(:));
            PDIeqNoise=log(PDIeqNoise);
            ...
            ax(1)=subplot(3,4,1);
            imagesc(1*unNorm_CBF_speedInd(:,:,1));
%             colormap(ax(1),VzCmap)
            colormap(ax(1),hot)
            caxis(speedAxisRange)
            title('CBF-speed Index, negFreq')
            ...
            ax(5)=subplot(3,4,5);
            imagesc(1*unNorm_CBF_speedInd(:,:,2));
%             colormap(ax(5),VzCmap)
            colormap(ax(5),hot)
            caxis(speedAxisRange)
            title('CBF-speed Index, posFreq')
...
%             colormap(ax(10),VzCmap)
            ...
            ax(3)=subplot(3,4,3);
            imagesc(unNorm_CBVeqNoise(:,:,1));
            caxis(CBVAxisRange)
            colormap(ax(3),gray)
            title('CBV Index,negFreq')
            
            ax(7)=subplot(3,4,7);
            imagesc(unNorm_CBVeqNoise(:,:,2));
            colormap(ax(7),gray)
            caxis(CBVAxisRange)
            title('CBV Index, posFreq')
            
            ax(11)=subplot(3,4,11);
            imagesc(unNorm_CBVeqNoise(:,:,3));
            colormap(ax(11),gray)
            caxis(CBVAxisRange)
            title('CBV Index, allFreq')
...
%% figure plot - descending and ascending fused results
% [VzCmap]=Colormaps_fUS;
