function varargout = GUI_US_Analysis(varargin)
% GUI_US_ANALYSIS MATLAB code for GUI_US_Analysis.fig
%      GUI_US_ANALYSIS, by itself, creates a new GUI_US_ANALYSIS or raises the existing
%      singleton*.
%
%      H = GUI_US_ANALYSIS returns the handle to a new GUI_US_ANALYSIS or the handle to
%      the existing singleton*.
%
%      GUI_US_ANALYSIS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_US_ANALYSIS.M with the given input arguments.
%
%      GUI_US_ANALYSIS('Property','Value',...) creates a new GUI_US_ANALYSIS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_US_Analysis_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_US_Analysis_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help GUI_US_Analysis

% Last Modified by GUIDE v2.5 14-Aug-2019 15:33:27

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_US_Analysis_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_US_Analysis_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_US_Analysis is made visible.
function GUI_US_Analysis_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_US_Analysis (see VARARGIN)

% Choose default command line output for GUI_US_Analysis
...
% UIWAIT makes GUI_US_Analysis wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_US_Analysis_OutputFcn(hObject, eventdata, handles) 
...
axes(handles.axes1)
imagesc(log(abs((handles.PDI(:,:,3))))); colorbar; 
colormap(handles.axes1, hot)
guidata(hObject, handles);
...
axes(handles.axes1)
imagesc((abs((handles.GG(:,:,iTau))))); colorbar; 
