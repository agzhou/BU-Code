...
% PRSinfo.Dim: GG original dimension: [nz,nx,ny,nTau]
% output:
    % Ms, Mf, R, [nz,nx,ny]
    % Vt, mm/s, [nz,nx,ny]
    % Vz, mm/s, [nz,nx,ny]
...
% [Ms, Mf, Vt, Vz, D, R, GGf]=GG2VDR_GPU(GG, PRSinfo); 
...
R=gather(reshape((1-sum(abs(GG-GGf).^2,2)./sum(abs((GG)-mean(GG,2)).^2,2)).*CR,[nz,nx,ny]));  
