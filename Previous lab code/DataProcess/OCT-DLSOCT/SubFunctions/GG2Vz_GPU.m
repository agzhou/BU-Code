...
% nItp=10;
% [Vz]=GG2Vz_GPU(GG, PRSinfo, nItp);  % m/s
...
for iChk=1:nyChk
    iGG=gpuArray(GG((iChk-1)*nVoxPchk+1:iChk*nVoxPchk,:));
    sTau=linspace(1,nTau,nTau).';
    rsTau=linspace(1,nTau,nTau*nItp).';
    GGI=movmean(interp1(sTau,imag(iGG).',rsTau,'linear'),10,1); %[nTau, nVox]
    ...
    Vz((iChk-1)*nVoxPchk+1:iChk*nVoxPchk,1)=gather(((PRSinfo.Lam(1)./(2*n*Tvz).*sign(mean(GGI(1:3,:),1)).*CR).')); % absolute value of axial velocity, m/s
end
