%% DLSOCT data processing, Repeat Ascan, CPU-based
% input: 
...
prompt1={['Num Ysegs (nY=',num2str(Dim.ny),', nX=',num2str(Dim.nx),', NxRpt=',num2str(Dim.nxRpt),', NyRpt=',num2str(Dim.nyRpt),')'],...
    ...
    'intDk(-0.19 for BUData, -0.235 for BU Awake, -0.21 for BU-PMP, -0.43 for MGH Anes, -0.36 for MGH Awake)','Aline rate (kHz)','FWHM-Trans (um)', 'FWHM-Axial (um)'};
...
RR = DAT2RR(DAT,PRSinfo.intDk);  % CPU
...
for iChk=1:N_YChks
    ...
    RR0 = DAT2RR(DAT, PRSinfo.intDk);         % process raw spectrum data to spatial reflectivity data 
%     RR0 = DAT2RR_GPU(DAT, intDk);     % process raw spectrum data to spatial reflectivity data 
    ...
    GG(:,:,(iChk-1)*nyPerChk+1:iChk*nyPerChk,:)=RR2g1(RR, PRSinfo); % calculate g1, CPU-based
...
for iChk=1:N_YChks
    ...
    [iMs, iMf, iVt, iVz, iD, iR]=GG2VDR(iGG, PRSinfo);
...
% %% 3.3 GG2Vz0
% [Vz0]=GG2Vz(reshape(GG,[LengthZ*Dim.nx*Dim.ny,PRSinfo.g1_ntau]), PRSinfo, nItp);
% % [Vz0]=GG2Vz_GPU(reshape(GG,[LengthZ*Dim.nx*Dim.ny,PRSinfo.g1_ntau]), PRSinfo, nItp);
% Vzi=reshape(Vz0,[LengthZ,Dim.nx,Dim.ny]);
%% SAVE RESULTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
save([datapath, 'DLSOCT-CPU.mat'],'-v7.3','D','Vt','Vz','Ms','Mf','R','PRSinfo')
