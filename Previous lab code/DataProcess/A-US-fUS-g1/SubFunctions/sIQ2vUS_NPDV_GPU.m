...
function [Mf, Vz, V, pVz, Vcz, R, CR, Vx, Ms, pnRatio,GGf]=sIQ2vUS_NPDV_GPU(sIQ, PRSSinfo)
%% O. constant
...
if nz0*nx0==1
    ...
    gRm=mean(gR(:));
    gRstd=std(gR(:));
    gCR=(gR>max((gRm-0.4*gRstd),0.08))+(gR<=max((gRm-0.4*gRstd),0.08)).*gR*5;
    gCR(gCR<0.1)=0;
    ...
    aCR=(((fSNR0>fSNRthd0)+abs(GG0(:,:,1))>0.4)>0).*...
        (PRSSinfo.useMsk*1+(1-PRSSinfo.useMsk)*gCR); % all frequency signal-based thresholding
end
clear GG0 sIQ
...
for iNP=1:2
    ...
    %% V. vUS fitting  
    % V.3. g1(1) adjust
    ggCR1=(abs(abs(GG2(:,1))-abs(GG2(:,2)))>2*abs((abs(GG2(:,2))-abs(GG2(:,3))))).*(abs(abs(GG2(:,1))-abs(GG2(:,2)))>0.15);
    ggCR2=(abs(GG2(:,1))>0.55).*(abs(GG2(:,2))<0.25).*(abs(GG2(:,2))<abs(GG2(:,3)));
    ggCR0=((ggCR1+ggCR2)>0);
    GG2(:,1)=(1-ggCR0).*GG2(:,1)+ggCR0.*(min(real(GG2(:,2))+abs(real(GG2(:,2)-GG2(:,3)))*1.2,1)+...
        1i*((imag(GG2(:,2))-imag(GG2(:,2))*2/3)));
