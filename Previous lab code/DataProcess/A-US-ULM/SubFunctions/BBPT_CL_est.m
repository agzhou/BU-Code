...
%     CoorBB{it}=[zBB,xBB];
% end
function [BBPD]=BBPT_CL_est(CoorBB,PRSSinfo) % use the coordinates of BB
nt=length(CoorBB);
...
%                 dCrit=(PRSSinfo.dCrit(1)/PRSSinfo.lPix);% maximum distance between BB(t+1) and BB(t) 
                dCrit=max(min((mV)*2,8),12);% maximum distance between BB(t+1) and BB(t) 
                dEstCrit=min(max(mV*1.5,1),8); % maximum distance between BBest(t+1) and BB(t+1) 
%                 dCrit=max(min((mV)*1,1),3);% maximum distance between BB(t+1) and BB(t) 
%                 dEstCrit=min(max(mV*1,1),3); % maximum distance between BBest(t+1) and BB(t+1) 
            end
            %% pairing frames
...
%             hold on,plot(iBBCoorCBB(2),iBBCoorCBB(1),'r<'); hold on,plot(estCoorNPD(2),estCoorNPD(1),'bo');
            ...
            if ~isempty(dBBnext)
                ...
                if (MutualMin==0) 
                    v=sqrt(sum(((iBBCoorCBB-iBBCoorNPD).^2),2))*PRSSinfo.lPix/1e3*PRSSinfo.CCFR/(nMis+1);
