...
P.numSupFrames=str2num(inputValue{2});
P.PDIFR=str2num(inputValue{3});
P.numCCframes=str2num(inputValue{4});
P.CCFR=str2num(inputValue{5});
...
P.tIntPlane=33;    % us, time interval between successive plane wave, travel time + system overhead time, the default is 33 us for 150 wavelengths acquisition
...
for iSuperFrame = 1:Resource.RcvBuffer(1).numFrames
    ...
    Event(n-1).seqControl = [6,4];
    %% SubFrame DMA
    Event(n-round(P.numCCframes*P.numAngles/2)).seqControl = [3, nsc]; % DMA after 50th acquisition
       SeqControl(nsc).command = 'transferToHost'; % transfer frame to host buffer
       nsc = nsc+1;
 
%     Event(n-2001).seqControl = [1, nsc]; % DMA after 50th acquisition
%        SeqControl(nsc).command = 'transferToHost'; % transfer frame to host buffer
%        nsc = nsc+1;
%        
%     Event(n-4001).seqControl = [1, nsc]; % DMA after 50th acquisition
%        SeqControl(nsc).command = 'transferToHost'; % transfer frame to host buffer
%        nsc = nsc+1;
