...
prompt={'Processing start file','number of files to process','PDelta-X',...
         'PDelta-Z'};
name='Beamforming';
defaultvalue={BFinfo.fileInfo{8}(1:end-4),'1','0.5','0.5'};
numinput=inputdlg(prompt,name, 1, defaultvalue);
BFinfo.startFile=str2num(numinput{1});
BFinfo.nFile=str2num(numinput{2});
BFinfo.PDeltaX=str2num(numinput{3});
BFinfo.PDeltaZ=str2num(numinput{4});
BFinfo.PreFile=BFinfo.startFile;

save(['D:\CODE\Mains\','BFinfo.mat'],'BFinfo')
for iFile=BFinfo.PreFile:BFinfo.startFile+BFinfo.nFile-1
    ...
    %     BFinfo.IQdataFilename = [iFileName(1:end-6),'IQ.mat'];
    iFileName=[strjoin(BFinfo.fileInfo(1:7),'-'),'-',num2str(iFile),'.mat'];
    %     BFinfo.IQdataFilename = ['IQ-',iFileName(1:end-6)];
    BFinfo.IQdataFilename = ['IQ-',iFileName(4:end)];
    save(['D:\CODE\Mains\','BFinfo.mat'],'BFinfo','iFile')
    %%
    ...
    if simulateMode ==2
        disp('Loading Data...')
        load ([BFinfo.FilePath, iFileName]);  % this file includes RcvData and the numAcqs and numFrames parameters for properly setting Resources
        disp('Data Loaded!')
        RcvData{1}=RFRAW;
        clear RFRAW RFRAW0;
    else
    ...
    if simulateMode == 2
        ...
        %                 NewPlane=NewPlane+1;
        %                 iFileName=[strjoin(fileInfo(1:6),'-'),'-',num2str(NewPlane),'-',strjoin(fileInfo(8:end),'-')];
        %                 IQdataFilename = [iFileName(1:end-6),'IQ.mat'];
        %             end
        %         end
        ...
        save ([BFinfo.FilePath,BFinfo.IQdataFilename], 'IQ', 'P')
    end
