...
if simulateMode ==2
    ...
    DAQInfo.savepath=FilePath;
    FileInfo=strsplit(FileName,'-');
    DAQInfo.Filename=FileInfo{7};
    DAQInfo.FileNameFull=FileName;
    save('D:\CODE\Mains\DAQParameters.mat','DAQInfo','P');
    %% to remove high amplitude signal 
%     RFRAW0=reshape(RFRAW,[P.actZsamples,P.numAngles,P.numCCframes,P.nCh]);
%     for iAgl=1:P.numAngles;
%         for iCh=1:P.nCh;
%             RFRAW0(abs(squeeze(mean(RFRAW0(:,iAgl,:,iCh),3)))>mean(abs(squeeze(mean(RFRAW0(:,iAgl,:,iCh),3))))*5,iAgl,:,iCh)=0;
%         end;
%     end
%     RFRAW=reshape(RFRAW0,[P.actZsamples*P.numAngles*P.numCCframes,P.nCh]);
    %%
    RcvData{1}=RFRAW;
    clear RFRAW RFRAW0;
    %% 1.2. IQ beamforming parameter
    prompt={'PDelta-X',...
        'PDelta-Z'};
...
%     FilePath='E:\0124 Pantom Experiment\yResolutionAgl20\';
%     FileName='Agl20-20CC-500Hz-20v-nA11Cd14-38.mat';
...
tCCUS=round(1e6/P.CCFR);
%%%% Img and IQ data save name
...
Resource.Parameters.numRcvChannels = nRcv;   % number of receive channels.
...
PData(1).PDelta = [Trans.spacing*PDeltaX, 0, PDeltaZ];
PData(1).Size(1) = ceil((EndDepth-StartDepth+1)/PData(1).PDelta(3)); % startDepth, endDepth and pdelta set PData(1).Size.
PData(1).Size(2) = ceil((Resource.Parameters.numRcvChannels*Trans.spacing)/PData(1).PDelta(1));
PData(1).Size(3) = 1;      % single image page
PData(1).Origin = [-Trans.spacing*(Resource.Parameters.numRcvChannels-1)/2,0,StartDepth]; % x,y,z of upper lft crnr.
% No PData.Region specified, so a default Region for the entire PData array will be created by computeRegions.
P.dxImg=Trans.spacingMm/(Trans.spacing/PData(1).PDelta(1));
P.xCoor=linspace(0,PData(1).Size(2)*P.dxImg,PData(1).Size(2)); % Reconstructed image xCoor, mm
P.dzImg=PData(1).PDelta(3)*P.wavelength;
P.zCoor=linspace(PData(1).Origin(3)*P.wavelength,PData(1).Origin(3)*P.wavelength+PData(1).Size(1)*P.dzImg,PData(1).Size(1)); % Reconstructed image zCoor, mm
...
Resource.InterBuffer(1).numFrames = P.numCCframes;  % for all acquired data reconstruction
Resource.ImageBuffer(1).numFrames = P.numCCframes*P.numSupFrames; % for all acquired data reconstruction
...
P.IDdelta=Resource.DisplayWindow(1).pdelta;
P.IDxCoor=linspace(1*P.IDdelta*P.wavelength,DwWidth*P.IDdelta*P.wavelength,DwWidth);
P.IDzCoor=P.zCoor(1)+linspace(1*P.IDdelta*P.wavelength,DwHeight*P.IDdelta*P.wavelength,DwHeight);
...
% emitElem(15:114)=1;
% emitElem = [zeros(1,(128-nTrans)/2),emitElem,zeros(1,(128-nTrans)/2)];
...
TX((P.numAngles+1)/2).Steer = [0.25/180*pi,0.0];
TX((P.numAngles+1)/2).Delay = computeTXDelays(TX((P.numAngles+1)/2));
P.TXDelay((P.numAngles+1)/2,:)=TX((P.numAngles+1)/2).Delay;  % avoid 0 angle
...
rcvElem=ones(1,128);%Resource.Parameters.numRcvChannels);
...
Recon = repmat(struct('senscutoff', 0.6, ...
    ...
    'RINums', 1:P.numAngles),1,P.numCCframes);
%%%% Define ReconInfo structures.
ReconInfo = repmat(struct('mode', 'accumIQ', ...  % default is to accumulate IQ data.
    'txnum', 1, ...
    'rcvnum', 1, ...
    'regionnum', 1), 1,P.numAngles*P.numCCframes);

%%%% - Set specific ReconInfo attributes.
for iCCframe=1:P.numCCframes
    %%%% - Set specific Recon attributes.
    Recon(iCCframe).IntBufDest=[1,iCCframe];
    Recon(iCCframe).RINums=(iCCframe-1)*P.numAngles+1:iCCframe*P.numAngles;
    %%%% - Set specific ReconInfo attributes.
    ReconInfo(P.numAngles*(iCCframe-1)+1).mode = 'replaceIQ';
    for iAngle = 1:P.numAngles
        iAcq=P.numAngles*(iCCframe-1)+iAngle;
        ReconInfo(iAcq).txnum = iAngle;
        ReconInfo(iAcq).rcvnum = iAcq;
    end
    ReconInfo(P.numAngles*iCCframe).mode = 'accumIQ_replaceIntensity';
end
...
Process(1).Parameters = {'imgbufnum',1,...   % number of buffer to process.
                         ...
                         'displayWindow',1};
% Define the external processing event for Image display data saving. 
Process(2).classname = 'External';
Process(2).method = 'SaveDisplayData';
Process(2).Parameters = {'srcbuffer','imageP',... % name of buffer to process.
                        'srcbufnum',1,... 
                        'srcframenum',1,...
                        'dstbuffer','none'};
...
SeqControl(6).argument = min(P.tIntPDI-tCCUS*P.numCCframes+10,4e6);  % in us; time for data trasfer to host, amd to next superframe/PDI acquisition
nsc = 7; % nsc is count of SeqControl objects

%% 15. Event objects to acquire all acquisitions
n = 1; % n is count of Events
for iSuperFrame = recSupFrame
%     for iCCframe = 1:P.numCCframes
%         for iAngle=1:P.numAngles
...
%         nsc = nsc + 1;
    ...
    for iCCframe=1:P.numCCframes
%         iCCframeAll=P.numCCframes*(iSuperFrame-1)+iCCframe;
        Event(n).info = 'Reconstruct';
        Event(n).tx = 0;
        Event(n).rcv = 0;
        Event(n).recon = iCCframe;
        Event(n).process = 1;
%         Event(n).process = 2;
...
%     For live acquisition in mode 0, simply comm4ent out the 'if/end' statements and manually freeze and exit when the data looks good.
...
%     save ([FilePath,IMGdataFilename], 'Img', 'P')
    save ([FilePath,IQdataFilename], 'IQ', 'P')
end
FileName
