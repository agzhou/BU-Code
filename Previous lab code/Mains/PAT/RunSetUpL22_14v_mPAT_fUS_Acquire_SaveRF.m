...
Process(2+P(2).numSupFrames).classname = 'External';
Process(2+P(2).numSupFrames).method = 'SaveRFonline';
Process(2+P(2).numSupFrames).Parameters = {'srcbuffer','receive',... % name of buffer to process.
    'srcbufnum',1,... 
    'srcframenum',1,... 
    'dstbuffer','none'};
...
for iSuperFrame = 1:Resource.RcvBuffer(1).numFrames
    ...
    for iFrame_pa = (P(1).numAcqs+1):(P(1).numAcqs+P(2).numAcqs)
       % Pause for optical buildup
        ...
        Event(n).info = 'Wait for Trigger IN and noop';
        Event(n).tx = 0;
        Event(n).rcv = 0;
        Event(n).recon = 0;
        Event(n).process = 0;
        Event(n).seqControl = [9,10]; %wait for trigger in, timeout
        n = n+1;
     ...
     Event(n-1).seqControl = [4,12,7]; % 12% replace last PA acquisition Event's seqControl with longer TTNA and RCV profile change
    ...
    Event(n-1).seqControl = [6,8,nsc]; % time to superframe  Set last US acquisition Event's seqControl (longer TTNA and new RCV profile)
    ...
    Event(n).process = 2+P(2).numSupFrames;  % call SaveRFonline
    n = n+1;
