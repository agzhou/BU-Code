...
save('D:\CODE\Mains\PAT\PATAQParameters_L22_14v.mat','PATAQInfo');
...
PData.PDelta = [Trans.spacing*0.5, 0, 0.5];
...
Process(2).method = 'SavePARFonline';
...
% Process(2).classname = 'External';
% Process(2).method = 'SavePAImgonline';
...
Event(num_evt).info = 'Call external processing function';
Event(num_evt).tx = 0;            % no TX structure.
Event(num_evt).rcv = 0;           % no Rcv structure.
Event(num_evt).recon = 0;         % reconstruction.
Event(num_evt).process = 2;       % reconstruction processing

% Event(num_evt).info = 'Call external processing function Img';
% Event(num_evt).tx = 0;            % no TX structure.
% Event(num_evt).rcv = 0;           % no Rcv structure.
% Event(num_evt).recon = 0;         % reconstruction.
% Event(num_evt).process = 3;       % reconstruction processing
% Event(num_evt).seqControl = 5;    % return to matlab
num_evt = num_evt + 1;
