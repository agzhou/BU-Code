...
%% Save parameters
load('H:\PAT_L123v_mice\mice1_10_09_20\RF-IQ-PAT800-CPRH1-3');
PATAQInfo=P;
...
Resource.Parameters.simulateMode = 2; % Resource.Parameters.simulateMode = 2 stops sequence and processes RcvData continuously.
...
actAcqLength =(maxAcqLength-P.StartDepth/2);
...
for iAcq=1:P.AverageNumber
%     Event(num_evt).info = 'Wait for Flashlamp IN and noop';
...
% %     Event(num_evt).seqControl = [2,7]; % wait for trigger in (= Flashlamp IN) then noop for > 300 us
%     Event(num_evt).seqControl = 2; % wait for trigger in (= Flashlamp IN) then noop for > 300 us
%     num_evt = num_evt + 1;

%     Event(num_evt).info = 'Send Q-Switch Signal and noop (2)';
%     Event(num_evt).tx = 0;              % no TX structure.
%     Event(num_evt).rcv = 0;             % no Rcv structure.
%     Event(num_evt).recon = 0;           % no reconstruction.
%     Event(num_evt).process = 0;         % no processing
%     Event(num_evt).seqControl = [3, 8];      % send trigger out
%     num_evt = num_evt + 1;

    Event(num_evt).info = 'Start Acquisition';
    Event(num_evt).tx = 0;              % no TX structure.
    Event(num_evt).rcv = 0;          % use 1st Rcv structure.
    Event(num_evt).recon = 1;           % no reconstruction.
    Event(num_evt).process = 1;         % no processing
    Event(num_evt).seqControl = 2; %  wait for trigger in to acquire + send trigger Out %+ transfer RF data to host
    num_evt = num_evt + 1;
end
...
SeqControl(4).argument = 10e6; % Wait 10 seconds

% SeqControl(7).command = 'noop';
% SeqControl(7).argument = round(QSwitch_delay_us*5);
% 
...
if P.simulateMode ~= 2    
    ...
    if exist([savepath,RFdataFilename,'.mat'])
        ...
        while (exist([savepath,RFdataFilename,'.mat'])==2)
            NewPlane=NewPlane+1;
            RFdataFilename = ['RF-',P.Filename,'-',num2str(NewPlane)];
        end
        ...
        RFdataFilename = ['RF-',P.Filename,'-',num2str(P.Plane)];
    end
    tic
