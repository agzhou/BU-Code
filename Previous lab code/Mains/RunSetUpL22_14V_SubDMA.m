...
%   for its use
%
% File name: SetUpL11_5v_128RyLnsSubFrmDma.m - Example of the sub-frame DMA
% feature
% Description: 
%   This is a copy of the L11-5v_128RyLns script, modified to add subframe
%   DMA transfers within the acquisition event sequence for each frame.
%   The changes made to the script are highlighted with the comment line:
%      % ****** Sub Frame *******
% 
%
% Last update:
% 4/5/2017 created for subframe DMA feature in 3.2.2 release
clear all
...
% this command will display CGD memory allocation at the Matlab command
% prompt when VSX loads and initializes the script.  The value
% "numberOfAcquisitonBlocks" represents the amount of memory allocated
% within each channel group for storage of receive data.  One acquisition
% block is nTransEvent(eg. 128) samples per channel (and thus the size of an acquisition
% block is 8192 bytes = 128 samples * 2 bytes per sample * 32 channels).
% Thus 128 * numberOfAcquisitionBlocks will give you the total number of
% samples being allocated for each individual channel.
...
PData(1).Region = repmat(struct('Shape',struct( ...
                    ...
                    'width',Trans.spacing,...
                    'height',P.endDepth-P.startDepth)),1,nTransEvent);
...
Resource.RcvBuffer(1).rowsPerFrame = 4096*nTransEvent;   % this size allows for maximum range
Resource.RcvBuffer(1).colsPerFrame = Resource.Parameters.numRcvChannels;
Resource.RcvBuffer(1).numFrames = 2;       % 20 frames used for RF cineloop.
Resource.InterBuffer(1).numFrames = 1;  % one intermediate buffer needed.
Resource.ImageBuffer(1).numFrames = 10;
Resource.DisplayWindow(1).Title = 'L11-5v_128RyLnsSubFrmDma';
...
TX = repmat(struct('waveform', 1, ...
                   ...
                   'Delay', zeros(1,Trans.numelements)), 1, nTransEvent);          
% - Set event specific TX attributes.
for n = 1:nTransEvent   % nTransEvent transmit events
...
% - We need nTransEvent Receives for every frame.
...
Receive = repmat(struct('Apod', ones(1,Trans.numelements), ...
                        ...
                        'sampleMode', 'NS200BW',...
                        'mode', 0, ...
                        'callMediaFunc', 0), 1, nTransEvent*Resource.RcvBuffer(1).numFrames);
...
n = 1;
for i = 1:Resource.RcvBuffer(1).numFrames
    for j = 1:nTransEvent                      % Acquire frame
        Event(n).info = 'Aqcuisition.';
        Event(n).tx = j;   
        Event(n).rcv = nTransEvent*(i-1)+j;   
        Event(n).recon = 0;      
        Event(n).process = 0;    
   ...
   % ****** Sub Frame *******
   % Insert two sub frame DMA transferToHost commands in the previous nTransEvent
   % events representing the acquisition of one frame.  Note that you can
   % insert any number of sub frame transfers, and the spacing between them
   % does not need to be uniform.  In this example we will insert them
   % after acqNum's 50 and 95 of the 128 acquisition events.
   Event(n-round(nTransEvent/2)).seqControl = [1, nsc]; % DMA after 50th acquisition
       SeqControl(nsc).command = 'transferToHost'; % transfer frame to host buffer
       nsc = nsc+1;
       
%    Event(n-24).seqControl = [1, nsc]; % DMA after 95th acquisition
%        SeqControl(nsc).command = 'transferToHost'; % transfer frame to host buffer
%        nsc = nsc+1;
   % Adding these subframe transferToHost commands in the acquisition event
   % sequence is the only change needed to add the subframe feature to a
   % script
