...
SeqControl(7).command = 'triggerIn';  % time between frames
SeqControl(7).condition = 'Trigger_2_Rising';  % Wait for trigger 1 rising edge

SeqControl(8).command = 'sync';
SeqControl(8).argument = 60e6; % Wait 60 seconds
...
if simulateMode==2 || simulateMode==0 %  In live acquisiton or playback mode, run continuously, but run only once for all frames in simulation
    ...
    Event(n).seqControl = 8;    % make the Sw wait at most 60s for the Hw to sync
    n=n+1;
