...
% DefaultPath='Z:\fUS Data\';
load('D:\CODE\Mains\DataProcessingPath.mat');
...
BFinfo.FolderInfo=strsplit(BFinfo.FilePath,'\'); % File info
FileFolder=BFinfo.FolderInfo{end-1};
BFinfo.FolderBase=FileFolder(isletter(FileFolder))
startFolder=regexp(BFinfo.FolderInfo{end-1},'\d+','match');
startFolder=startFolder{1};
DefaultPath=BFinfo.FilePath;
save('D:\CODE\Mains\DataProcessingPath.mat','DefaultPath')
%%
prompt={'Processing start file','nRepeat to process','CP start', 'nCP','PDelta-X',...
         'PDelta-Z','Start Folder','nFolder'};
name='Beamforming';
defaultvalue={BFinfo.fileInfo{8}(1:end-4),'1','1','1','0.5','0.5',startFolder,'1'};
...
BFinfo.startFolder=str2num(numinput{7});
BFinfo.nFolder=str2num(numinput{8});
BFinfo.PreFile=BFinfo.startFile;
BFinfo.PreFolder=BFinfo.startFolder;
%% CP/file name 
BFinfo.subName=[BFinfo.startCP:BFinfo.startCP+BFinfo.nCP-1];
%% Beamforming
for iFolder=BFinfo.PreFolder:BFinfo.startFolder+BFinfo.nFolder-1
    BFinfo.PreFolder=iFolder+1;
    BFinfo.FolderInfo{end-1}=[BFinfo.FolderBase, num2str(iFolder)];
    BFinfo.iFilePath=strjoin(BFinfo.FolderInfo,'\');
    BFinfo.PreCP=1;
    if exist(BFinfo.iFilePath, 'dir')==7
        for iCP=BFinfo.PreCP:BFinfo.nCP
            ...
            for iFile=BFinfo.PreFile:BFinfo.startFile+BFinfo.nFile-1
                ...
                if simulateMode ==2
                    ...
                    tic;
                    load ([BFinfo.iFilePath, BFinfo.iFileName]);  % this file includes RcvData and the numAcqs and numFrames parameters for properly setting Resources
                    disp(['Data Loaded! - ',datestr(datetime('now'))])
                    RcvData{1}=RFRAW;
                    clear RFRAW RFRAW0;
                else
                ...
                PData(1).PDelta = [Trans.spacing*BFinfo.PDeltaX, 0, BFinfo.PDeltaZ];
                ...
                TGC.CntrlPts = [700,830,900,950,995,1000,1000,1000];
                ...
                %     SeqControl(2).argument = P.tIntPlane;  % in usec, time between successive angles
                ...
                filename='VSXBF_MultiFile';
                save(['MatFiles/',filename]);
                disp([ filename ': NOTE -- Running VSX automatically!'])
                VSXBF
                ...
                if simulateMode == 2
                    disp ('Info:  Saving the Img Data and IQ Data buffers -- please wait!')
                    load(['D:\CODE\Mains\','BFinfo.mat']);
                    ...
                    save ([BFinfo.iFilePath,BFinfo.IQdataFilename], 'IQ','Img', 'P')
                end
                toc
                disp([BFinfo.IQdataFilename,' is processed! - ',datestr(datetime('now'))]);
                disp(' ')
