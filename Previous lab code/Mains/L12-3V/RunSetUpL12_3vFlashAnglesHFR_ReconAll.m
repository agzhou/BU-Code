% Notice: 
%
% File name: runSetUpL12_3vFlashAnglesHFR.m 
% - High frame rate Coherence Compounding plane wave US imaging
% - use the central portion 128 elements of L12_3v transdcuer (192 elements total)
% Description: 
%   Sequence programming file for L12-3v Linear array, using the central 128 elements
%   plane wave transmits with multiple steering angles on a 256  
%   channel system (use only one connector). 128 transmit channels and 128 receive channels are  
%   active and positioned as follows (each char represents 4 elements) 
%
%   Element Nos.                                1              1
%                       3       6       9       2              9
%               1       3       5       7       9              2
...
%
%   The receive data are stored under  
%   different acqNums in the Receive buffer. The reconstruction sums the 
%   IQ data from the multi angle aquisitions and computes intensity values to produce 
...
defaultpath='C:\Users\verasonics\Documents\Vantage-3.4.0-1711281030\MatFiles\';
...
if simulateMode ==2,
    ...
    P.numFrames = 10;      % no. of super frames (real-time images are produced 1 per frame)
...
IMGdataFilename = [FileName(1:end-4),'-IMG.mat'];
IQdataFilename = [FileName(1:end-4),'-IQ.mat'];
%% 2. System parameters
...
Resource.ImageBuffer(1).numFrames = P.numCCframes*P.numFrames; % for all acquired data reconstruction
...
Resource.DisplayWindow(1).numFrames = P.numFrames*P.numCCframes; % for displaying all acquired CCUS images
...
%         Recon(iCCframeAll).IntBufDest=[1,iCCframeAll];
%         Recon(iCCframeAll).RINums=(iCCframe-1)*P.numAngles+1:iCCframe*P.numAngles;
%     end
% end
...
SeqControl(2).argument = 40;  % 40 usec
...
if simulateMode == 2    
    ...
    save ([FilePath,IMGdataFilename], '-v7.3','ImgData', 'P')
    save ([FilePath,IQdataFilename], '-v7.3','IQData', 'P')
end
FileName
return
