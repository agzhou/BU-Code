...
TDanswer = questdlg('Use automatically-acquired stim timing data?', TDchoices{1}, TDchoices{2});
switch TDanswer
    case TDchoices{1}
        ...
        load(timingFilePath)
    case TDchoices{2}
        manualTimingPrompt = {'Baseline duration [s]', 'Stim duration [s]', 'Rest duration [s]', 'Number of trials'};
        manualTimingDefaults = {'5', '05', '20', '10'};
        ...
        initialVoltage = str2double(manualTimingUserInput{1});
        startDepthMM = str2double(manualTimingUserInput{2});
        endDepthMM = str2double(manualTimingUserInput{3});
        PRF = str2double(manualTimingUserInput{4});
        frameRate = str2double(manualTimingUserInput{5}); % subframe (volume) rate (Hz)
        na = str2double(manualTimingUserInput{6});
...
manualTimingPrompt = {'Start file number', 'End file number', 'SVD lower bound', 'SVD upper bound', 'Tau 1 index for CBFspeed', 'Tau 2 index for CBFspeed', 'Tau 1 index for CBV'};
manualTimingDefaults = {'1', '', '20', '1000', '2', '11', '2'};
manualTimingUserInput = inputdlg(manualTimingPrompt, 'Input Parameters', 1, manualTimingDefaults);

% define # of files manually for now
% str2double(parameterUserInput{});
startFile = str2double(manualTimingUserInput{1});
endFile = str2double(manualTimingUserInput{2});
numFiles = endFile - startFile + 1;
sv_threshold_lower = str2double(manualTimingUserInput{3});
sv_threshold_upper = str2double(manualTimingUserInput{4});
tau1_index_CBF = str2double(manualTimingUserInput{5});
tau2_index_CBF = str2double(manualTimingUserInput{6});
tau1_index_CBV = str2double(manualTimingUserInput{7});
