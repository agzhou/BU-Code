...
% SP.frameRate = 5000; % Frame rate [Hz]
...
voxel.size = [20e-3, 20e-3, 20e-3]; % Define x, y, z dimensions of the voxel
...
% Let x = [Ns, vx, vy, vz]
k = 2*pi/SP.wl; % Angular wavenumber
...
% abs_g1T_model = @(x, tau) abs( x(1)*Rs*(pi^1.5)*SP.sigma(1)*SP.sigma(2)*SP.sigma(3) ...
%                           / ( x(1)*Rs*(pi^1.5)*SP.sigma(1)*SP.sigma(2)*SP.sigma(3) + Re ) ...
%                           .* exp( -(x(2) .* tau).^2./(4.*SP.sigma(1)^2) -(x(3) .* tau).^2./(4.*SP.sigma(2)^2) -(x(4) .* tau).^2./(4.*SP.sigma(3)^2) ) ...
%                           .* exp(2.*1i.*k.*x(4).*tau) );
...
% testx = [1, 0, 0, SP.flow_v_mm_s * 2/1e3];
x0 = testx %%%%%%%% testing
lb = [0, 0, 0, 0]
ub = [Inf, 0, 0, 1000e-3]
...
options = optimoptions('lsqcurvefit', 'Algorithm', 'levenberg-marquardt', 'MaxIterations', 1000, 'FunctionTolerance', 1e-8); % lsqcurvefit options
% x_fit = lsqcurvefit(abs_g1T_model, x0, tau_range, abs(voxel.g1(tau_mask)), lb, ub)
x_fit = lsqcurvefit(abs_g1T_model, x0, tau_range, abs(voxel.g1(tau_mask)), lb, ub, options)
