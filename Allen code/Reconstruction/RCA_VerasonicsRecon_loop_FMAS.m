...
P.Media.attenuation = -0.5; % quick edit

% Mcr_savepath = [Mcr_datapath, 'IQ Data - Verasonics Recon\'];
Mcr_savepath = [Mcr_datapath, 'FMAS IQ Data - Verasonics Recon\'];
...
%     IQ = IData{1} + 1i .* QData{1};                                         % Merge the I and Q into one variable
    ...
    IQ_FMAS = DAS_to_FMAS(IData + 1i .* QData); % Convert to FMAS
    disp(strcat("IQ file ", num2str(Mcr_filenum), " converted to FMAS."))
    
%     savefast([Mcr_savepath, Mcr_IQfilenameStructure, num2str(Mcr_filenum)], 'IData', 'QData')

    % Save the I and Q separately because savefast doesn't support complex
    % values
    IData = real(IQ_FMAS);
    QData = imag(IQ_FMAS);
    savefast([Mcr_savepath, Mcr_IQfilenameStructure, num2str(Mcr_filenum)], 'IData', 'QData')
    disp(strcat("FMAS IQ file ", num2str(Mcr_filenum), " saved."))
