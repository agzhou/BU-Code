%%%% for running summmer school stuff overnight
clearvars
...
%% Load parameters, create save path, choose some options for recon
    % fpath = {'J:\Summer School 2026\08-03-2026\MS05\run2',...
    % 'J:\Summer School 2026\08-03-2026\MS05\run3',...
    % 'J:\Summer School 2026\08-03-2026\MS05\run4',...
    % 'J:\Summer School 2026\08-03-2026\wrist\test1_200'...
    % 'J:\Summer School 2026\08-03-2026\wrist\test2_400'};
    fpath = ['J:\Summer School 2026\08-05-2026\run' num2str(z)];
    % Mcr_datapath = uigetdir('G:\Allen\Data\', 'Select the raw data path');
    %Mcr_datapath = uigetdir('G:\', 'Select the raw data path');
    Mcr_datapath = fpath;
    datanum = length(dir([Mcr_datapath,'\RF*.mat']));
    Mcr_datapath = [Mcr_datapath, '\'];
    if ~exist('P', 'var')
        load([Mcr_datapath, 'params.mat']) % load acquisition parameters
    end
    
    Mcr_savepath = [Mcr_datapath,'IQ2'];
    ...
    Mcr_startFile = 1; %str2double(parameterUserInput{1}); % File to start reconstructing from
    Mcr_endFile = datanum;  % File to stop reconstructing on
    ...
    % 
    z_pix_spacing = Trans.spacing/2;
    x_pix_spacing = Trans.spacing/2;
    ...
    Recon = repmat(struct('senscutoff', sco, ... % Threshold for which the reconstruction doesn't consider an element's contribution due to directivity of the element, for a certain pixel (whose echoes are at an angle to the element). Should be in radians.
                   ...
                   'RINums', [1:na]), 1, 1); % The ReconInfo structure #(s). Each Recon must have its own unique set of ReconInfo #s
    ...
    ReconInfo = repmat(struct('mode', 'accumIQ', ... % reconstruct, and replace intensity data in ImageBuffer and IQ data in InterBuffer (see Table 12.4 in Tutorial)
                       ...
                       'regionnum', 1), 1, na);                % PData Region to process in
    ...
    Mcr_filename = 'L22_14v_Allen_recon_unstackedFrames.mat';
