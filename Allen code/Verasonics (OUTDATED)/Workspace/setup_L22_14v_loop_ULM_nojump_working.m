...
savepath = "G:\Allen\Data\01-15-2025 test\L22-14v\run 2\";
...
numSupFrames = 1;
numSubFrames = 100;
na = 11; % # of acquisitions per frame (# angles)
maxAngle = 5; % degrees
...
Receive = repmat(struct('Apod', ones(1, Trans.numelements), ...
                        ...
                        'LowPassCoef', [], ...
                        'InputFilter', []), 1, numBuffers * numSupFrames * numSubFrames * na);
...
for nbuf = 1:numBuffers
    Resource.RcvBuffer(nbuf).rowsPerFrame = numRcvSamples * na * 2 * numSubFrames;
    ...
    Resource.RcvBuffer(nbuf).numFrames = numSupFrames; % minimum # frames of RF data to acquire; RcvBuffer contains all the data needed for a whole frame, including multiple acquisition passes needed for reconstruction. Software can re-process RcvBuffer frames
...
% Process(1).method = 'ShowTimeTag'; % Function name
...
for nbuf = 1:numBuffers
    ...
    Process(nbuf + nprevproc).method = 'saveRcvData'; % Function name
...
%                 Event(n).rcv = (nsupf - 1) * na * numSubFrames + (nsubf - 1)*na + a; % Receive structure
                Event(n).rcv = (nbuf - 1) * numSupFrames * numSubFrames * na + (nsupf - 1) * numSubFrames * na + (nsubf - 1)*na + a; % Receive structure
...
%     Event(n).seqControl = [4, scInd, scInd-1]; % transfer and wait for the transfer %%%%% idk if this is correct %%%%%
