...
datapath = 'F:\Allen\Data\01-17-2025 AZ001 ULM\L22-14v\run 1 allen code left eye\Raw Data\';
...
%% Change gain
P.Media.attenuation = -0.5;

%% Stack RcvData files into one variable r (probably not needed)
...
% IQstack = []; % Create empty variable to append the data to (if you want
% a variable with every frame in the experiment)

% subfolderToSave = 'IQ data';
subfolderToSave = 'IQ data all half wl gain -0.5';
mkdir(datapath, subfolderToSave)
...
% for filenum = 1:2
    load([datapath, filename_structure, num2str(filenum), '.mat'])
    IQ = LA_DAS(RcvData, P, zPixSpacing, xPixSpacing);
...
%     IQstack = cat(4, IQstack, IQ); % concatenate along the frame dimension
    ...
    save([datapath, subfolderToSave, '\', filename_structure_IQ, num2str(filenum)], 'IQ', "-v6")
end

save([datapath, subfolderToSave, '\', 'pixSpacing'], 'zPixSpacing', 'xPixSpacing')
