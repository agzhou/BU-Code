% **** CHANGE FOR AUTOMATIC vsx CYCLING IS AT 'if runcount > 0' ****
...
% Default to not using FUS; if we load a FUS script it will
...
if ~exist('hwTestFlag', 'var')
    if Resource.SysConfig.HWconfigFault == 6
...
if exist('RcvData','var')
    ...
    if size(RcvData,1) ~= size(Resource.RcvBuffer,2)
        ...
        for i = 1:size(RcvData,1)
            ...
            if isfield(Resource.RcvBuffer(i),'gpuMemoryType')&&strcmp(Resource.RcvBuffer(i).gpuMemoryType,'GPU')
                clrRcvData = 1;
            end
...
if isfield(Trans,'HVMux')
    ...
    % check for HVMux SHIAperType (0 => Verasonics, 1 => GE)
    if ~isfield(Trans.HVMux,'SHIAperType') || isempty(Trans.HVMux.SHIAperType)
        Trans.HVMux.SHIAperType = 0.0; 
    end
...
if exist('UI','var')
    ...
    if ~isempty(userControlApp)
        % Use vsTempDir to support the MATLAB Compiler example code.
        uiControlParser = vsv.seq.uicontrol.UserUiParser(vsTempDir);
        userControlApp.setUiControlParser(uiControlParser);
...
% Conditionally set environment variables for running GPU Toolkit.
if isfield(Resource, 'RcvBuffer') && isfield(Resource.RcvBuffer, 'gpuMemoryType') || ...
        isfield(Resource, 'InterBuffer') && isfield(Resource.InterBuffer, 'gpuMemoryType') || ...
        isfield(Resource, 'ImageBuffer') && isfield(Resource.ImageBuffer, 'gpuMemoryType')
    vsv.gpu.getDefaultCudaLibPath();
end
...
%      'copyBuffers' - Copies back to Matlab IQData,ImgData,ImgDataP buffers
%          without running the Event sequence.
...
% make sure that runAcq was not yet executed
wasRunAcqExecuted = false;
...
while vsExit==0
    ...
    if autoScriptTest
        try
            ...
            if runcount > 0 %%%%%%%%%%%%%%%%%%% CHANGE TO AUTOMATE %%%%%%%%%%%%%%%%%%%
                vsExit = 1;
            end
        catch oopsy
            % Set the flag that says an exception occurred.
...
% Also check whether runAcq was executed at least one time. Otherwise we
% won't have buffers to copy
if ~isHwInFaultState && wasRunAcqExecuted
