...
Resource.RcvBuffer(1).rowsPerFrame = na*6144;
...
Resource.DisplayWindow(1).Title = 'GE9LDFlashAngles';
...
% TGC.CntrlPts = [153,308,710,960,960,960,960,960];
...
SeqControl(3).argument = 500;  % 100 msec (so 10 acq frames per second)
SeqControl(4).command = 'returnToMatlab';
SeqControl(5).command = 'noop';
SeqControl(5).argument = 1000;
...
%     n=n+1;
    ...
    if (floor(i/frameRateFactor) == i/frameRateFactor)&&(i ~= Resource.RcvBuffer(1).numFrames)  % Exit to Matlab
...
% - Range Change
MinMaxVal = [16,512,P.endDepth]; % default unit is wavelength
AxesUnit = 'wls';
...
% clearvars -except Process ImgData ImgDataP frameRateFactor maxAcqLength na ...
%     PData RcvData Receive Recon ReconInfo RcvProfile Resource SeqControl ...
%     TGC Trans TW TX TPC P startAngle dTheta
