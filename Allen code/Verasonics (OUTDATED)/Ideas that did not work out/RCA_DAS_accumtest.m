...
%     nzs_nopad = s(1) / na / 2;
    nzs_nopad = Receive(1).endSample; %%%%%%%% temp fix
    ...
    for f = 1:nf
        ...
        % first half of rcvs
        for a = 1:na
            d(:, :, a, f)  = r(Receive(a).startSample:Receive(a).endSample, numElements + 1 : numElements * 2, f);
        end
    
        % second half of rcvs
        for a = 1:na
%             d(:, :, na + a, f) = r(Receive(na + a).startSample:Receive(na + a).endSample, 1:numElements, f);
            d(:, :, na + a, f) = r(Receive(na_actual + a).startSample:Receive(na_actual + a).endSample, 1:numElements, f);
        end
    ...
    d_pad = zeros(nxs_nopad, round(startDepth * samplesPerWL), na * 2, nf);
...
%     znumpix = nzs_nopad;
    ...
    d_sum = zeros(xnumpix, ynumpix, znumpix, na*2, nf, 'single'); %%%%%%%%%%%%%%
    ...
    frame_ind_temp = zeros(numElements * nf * na * 2, 1);
    for f = 1:nf
        frame_ind_temp((f-1)*numElements * na * 2 + 1 : f * na * 2 * numElements) = f;
    end
    
    angles_ind_temp = zeros(numElements * na * 2, 1);
    % first half of angles
    for a = 1:na
        angles_ind_temp((a-1)*numElements + 1 : a * numElements) = a;
    end
    % second half of angles
    for a = 1:na
        angles_ind_temp(na * numElements + (a-1)*numElements + 1 : na * numElements + a * numElements) = na + a;
    end
...
%         assignStructVars(P)
    ...
    %         for xp = Media.MP(1, 1) + numElements/2 
            ...
            %     x_level_pix = (xp + xnumpix/2 - 1/2) / xnumpix * xsize;  % x value of xp in m. x = 0 is center of transducer array
                
            %     x_level_pix = (xp - xnumpix/2 + sign(xp - (xnumpix + 1)/2)* 1/2) / xnumpix * xsize;  % x value of xp in m. x = 0 is center of transducer array
    ...
    %             traveldistRX_temp = sqrt(z_level_pix ^ 2 + repmat((elementx - x_level_pix).^2, numElements, 1) + repmat((elementy - y_level_pix).^2, numElements, 1)');
                ...
                traveldistRX_temp = [repmat(traveldistRX_y_temp, 1, na) repmat(traveldistRX_x_temp, 1, na)];
    %             %%
    %             figure
    %             plot(traveldistRX_y_temp)
    %             hold on
    %             plot(traveldistRX_x_temp)
    %             hold off
    %             legend('y rot', 'x rot')
    %             %%
    ...
    %             traveldistTX_temp_y = z_level_pix .* cos(angles(1:na)) + (x_level_pix + sign(angles(1:na)) .* x_end) .* sin(angles(1:na)) + 1 * d_correction_1way;    % First half of acqs are for angles changing in y
    %             traveldistTX_temp_x = z_level_pix .* cos(angles(na + 1 : na*2)) + (sign(angles(na + 1 : na*2)) .* y_end - y_level_pix) .* sin(angles(na + 1 : na*2)) + 1 * d_correction_1way;    % Second half of acqs are for angles changing in x
    
    
                %             traveldistTX_temp_x = z_level_pix .* cos(angles) + (y_level_pix + sign(angles) .* y_end) .* sin(angles) + 1 * d_correction_1way;    % Second half of acqs are for angles changing in x
    %                     traveldistTX_temp = repmat([traveldistTX_temp_y, traveldistTX_temp_x], numElements, 1);
                traveldistTX_temp = repmat([traveldistTX_temp_y, traveldistTX_temp_x], numElements, 1);
    %             traveldistTX_temp_matrix = permute(repmat(traveldistTX_temp, numElements, 1, numElements), [1, 3, 2]);
    ...
    %             ind = fliplr(ind); % 9/19/24 correction
    %             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
                ia = repmat(reshape(ind, numElements*na * 2, 1), nf, 1); %%%% rearrange indices
                ib = repmat((1:numElements)', nf*na * 2, 1);
                ...
                dd = zeros(nxs, na*2, nf); 
                for f = 1:nf
                    % First half of angles (rotate about y)
                    for a = 1:na
    %                     dd(:, a, f) = d_delayed_temp((f-1)*na *
    %                     Trans.numelements + (a-1)*Trans.numelements + 1 : (f-1)*na*Trans.numelements + (a)*Trans.numelements); 
                        dd(:, a, f) = d_delayed_temp((f-1)*(na * 2) * numElements + (a-1)*numElements + 1 : (f-1)*(na * 2)*numElements + (a)*numElements);
        
                    end
                    % Second half of angles (rotate about x)
                    for a = 1:na
