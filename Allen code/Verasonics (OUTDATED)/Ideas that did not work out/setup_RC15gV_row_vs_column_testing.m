...
%% Description
% Testing R-R, C-C, C-R, and R-C pairs of TX-RX with 0 deg plane waves

% Acquisition and saving of RF data with the RC15gV probe
...
savepath = "G:\Allen\Data\11-15-2024 RC15gV row vs column tests\gel and small agarose phantom\trial 1 10v\";
% savepath = "D:\Allen\Data\11-07-2024 tests\RC15gV\trial 1\";
...
voltageLevel = 10; % Initial voltage level!!!

fps_target = 10;   % Intended frame rate

numChannels = 256; % enable all channels

numSupFrames = 1; % # of superframes, MUST BE ONE OR EVEN FOR VSX
numSubFrames = 4; % # of subframes
...
%% Define transmit power controller (TPC)
TPC(1).name = 'high voltage';
TPC(1).maxHighVoltage = Trans.maxHighVoltage;
TPC(1).highVoltageLimit = Trans.maxHighVoltage; % this one is supposed to change based on system status (like acq mode)
% TPC(1).hv = Trans.maxHighVoltage * 0.8; % set the initial value, with a safety factor
TPC(1).hv = voltageLevel; % set the initial value
...
% 2 types of transmissions of a plane wave: all rows or all columns
TX = repmat(struct('waveform', 1, ...
                   'focus', 0, ... % plane wave
                   'Steer', [0.0, 0.0], ... % theta, alpha (beam angle projected in xz from +z axis, beam angle wrt xz)
                   'Apod', zeros(1, Trans.numelements)), 1, 2);

txn = 1;

% Column
TX(txn).Apod(1:Trans.numelements/2) = ones(1, Trans.numelements/2); % Turn on columns (y)
TX(txn).Steer = [angles(1), 0];
TX(txn).Delay = computeTXDelays(TX(txn));

txn = txn + 1;
% Row
TX(txn).Apod(Trans.numelements/2 + 1 : end) = ones(1, Trans.numelements/2); % Turn on rows (x)
TX(txn).Steer = [0, angles(1)];
TX(txn).Delay = computeTXDelays(TX(txn));
...
%% Receiver array object (need numSubFrames = 4 for each row/column pair, per superframe)
...
Receive = repmat(struct('Apod', zeros(1, Trans.numelements), ... 
                        ...
                        'callMediaFunc', 0, ...
                        'LowPassCoef', [], ...
                        'InputFilter', []), 1, numSubFrames * numSupFrames);
j = 1;
% an = 0;
...
Resource.RcvBuffer(1).rowsPerFrame = nspa*na .* numSubFrames;
...
Resource.InterBuffer(1).pagesPerFrame = numSubFrames; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Process(2).method = 'testRcvData'; % Function name
% Process(1).Parameters = {'srcbuffer', 'bufferName', ...
%                          'srcbufnum', 1, ... % # of buffer to process
%                          'srcframenum', 1, ... % starting frame #
%                          'srcsectionnum', 1, ...
...
Resource.VDAS.dmaTimeout = 1000000;
% Flow:
% 1. Transmit (TX)
% 2. Receive (Receive)
...
SeqControl(scInd).command = 'timeToNextAcq'; % In us, allowed range is from 10 - 4190000
                                         ...
                                         % switch
timePerAcq = 1 / fps_target / (na * 1) * 1e6; % frame rate converted to acq time step (us)
...
for nsupf = 1:numSupFrames
    for nsubf = 1:numSubFrames
        ...
        n = n + 1;
        % Alternate between 1 and 2 TX structure
        switch nsubf
            case 1
                Event(n).info = 'C C';
                Event(n).tx = 1;
        ...
        Event(n).rcv = (nsupf - 1) .* numSubFrames .* na + (nsubf-1)*na + 1; % Use nth Receive structure % need to make this alternate between (1 and 2) * numframes or something
...
% save(strcat('MatFiles/', mfilename, '1'));
