%
% File name: SetUpGEL818iFlashAnglesPICollab_DoubleBmode.m - Example of 
% CEUS Pulse inversion imaging
%
% Description:
%   Sequence programming file for GEL818iD linear array, using flash 
%   transmits with multiple steering angles. This example uses 168 transmit 
...
%       TX-freq:5 MHz, PI TX-freq:5 MHz )
...
if (na > 1)
    dtheta = (12*pi/180)/(na-1); % set dtheta to range over +/- 6 degrees.
    startAngle = -6*pi/180;
else
    dtheta = 0;
    startAngle=0;
...
Resource.RcvBuffer(1).numFrames = 100;     % 20 frames for rcvDataLoop buffer.
...
save('MatFiles/GEL818iFlashAnglesPICollab_DoubleBmode');
