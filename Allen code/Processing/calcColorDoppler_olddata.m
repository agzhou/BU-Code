...
% For use with the old Bingxue/Jianbo parameter definition

% Output: CDI (same except Color Doppler index)
function [CDI] = calcColorDoppler_olddata(IQf_FT_separated, P)

    % Determine thresholds on the Fourier spectrum to get a more accurate
    % Doppler frequency estimation
    max_vz_expected = 0.05; % [m/s]
    abs_fD_max_expected = 2 * (P.frequency * 1e6) * max_vz_expected / P.vSound;
...
%         %%%%%%%%%%%%%%%%% currently not thresholding %%%%%%%%%%%%%%%%%%%%%
        ...
        CDI{i} = -fDi * P.vSound / 2 / (P.frequency * 1e6);
    end
