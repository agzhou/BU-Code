function varargout = roi_free(varargin)
...
%         eid = 'Images:roi_free:expectedHandle';
...
%     end
    ...
    otherwise
        CleanUp(xlimorigmode,ylimorigmode);
        eid = 'Images:roi_free:expectedFigureOrAxesHandle';
...
set(GETLINE_FIG, ...
    'Pointer', 'crosshair', ...
    'WindowButtonDownFcn', 'roi_free(''FirstButtonDown'');',...
    'KeyPressFcn', 'roi_free(''KeyPress'');', ...
    'DoubleBuffer', 'on');
...
case 'trap'
    ...
    eid = 'Images:roi_free:interruptedMouseSelection';
...
if (~strcmp(get(GETLINE_FIG, 'SelectionType'), 'normal'))
    ...
    set(GETLINE_FIG, 'WindowButtonMotionFcn', 'roi_free(''ButtonMotion'');', ...
            'WindowButtonDownFcn', 'roi_free(''NextButtonDown'');');
end
