function rect = slice_marker_horizontal(varargin)
...
%   $Revision: 5.22.4.5 $  $Date: 2005/11/15 00:58:17 $
...
if (nargin < 1)
    ...
    if (~ishandle(varargin{1}))
        CleanGlobals;
        eid = 'Images:slice_marker_horizontal:expectedHandle';
    ...
    otherwise
        CleanGlobals;
        eid = 'Images:slice_marker_horizontal:expectedFigureOrAxesHandle';
...
set(GETRECT_FIG, ...
    ...
    'WindowButtonMotionFcn', 'slice_marker_horizontal(''ButtonMotion'');', ...
    'WindowButtonUpFcn', 'slice_marker_horizontal(''ButtonUp'');', ...
    'DoubleBuffer', 'on');
...
case 'trap'
    ...
    eid = 'Images:slice_marker_horizontal:interruptedMouseSelection';
...
global GETRECT_FIG GETRECT_AX GETRECT_H1 GETRECT_H2
global GETRECT_PT1 GETRECT_TYPE x1 y1

set(GETRECT_FIG, 'Interruptible', 'off', ...
                 'BusyAction', 'cancel');


p=get(GETRECT_AX,'currentpoint');
axes_Lim = get(GETRECT_AX, {'XLim', 'YLim'});%抓取座標的XY limit
x1=p(1,1);
y1=p(1,2);


x1(x1<axes_Lim{1}(1))=axes_Lim{1}(1);%限定線段會在圖中
x1(x1>axes_Lim{1}(2))=axes_Lim{1}(2);%限定線段會在圖中
xdata = [x1 x1] ;
ydata = axes_Lim{2};
...
p=get(GETRECT_AX,'currentpoint');
axes_Lim = get(GETRECT_AX, {'XLim', 'YLim'});
x1=p(1,1);
y1=p(1,2);
x1(x1<axes_Lim{1}(1))=axes_Lim{1}(1);%限定線段會在圖中
x1(x1>axes_Lim{1}(2))=axes_Lim{1}(2);%限定線段會在圖中
...
set(GETRECT_FIG,'WindowButtonUpFcn', 'slice_marker_horizontal(''ButtonUp'');');
