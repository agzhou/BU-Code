function varargout = roi_poly(varargin)
...
%   $Revision: 5.27.4.4 $  $Date: 2005/11/15 00:58:15 $
...
%         eid = 'Images:roi_poly:expectedHandle';
...
%     end
    ...
    otherwise
        CleanUp(xlimorigmode,ylimorigmode);
        eid = 'Images:roi_poly:expectedFigureOrAxesHandle';
...
set(GETLINE_FIG, ...
    'Pointer', 'crosshair', ...
    'WindowButtonDownFcn', 'roi_poly(''FirstButtonDown'');',...
    'KeyPressFcn', 'roi_poly(''KeyPress'');', ...
    'DoubleBuffer', 'on');
...
case 'trap'
    ...
    eid = 'Images:roi_poly:interruptedMouseSelection';
...
if (~strcmp(get(GETLINE_FIG, 'SelectionType'), 'normal'))
    ...
    set(GETLINE_FIG, 'WindowButtonMotionFcn', 'roi_poly(''ButtonMotion'');', ...
            'WindowButtonDownFcn', 'roi_poly(''NextButtonDown'');');
end
