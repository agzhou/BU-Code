function rect = slice_marker_vertical(varargin)
...
if (nargin < 1)
    ...
    if (~ishandle(varargin{1}))
        CleanGlobals;
        eid = 'Images:slice_marker_vertical:expectedHandle';
    ...
    otherwise
        CleanGlobals;
        eid = 'Images:slice_marker_vertical:expectedFigureOrAxesHandle';
...
set(GETRECT_FIG, ...
    ...
    'WindowButtonMotionFcn', 'slice_marker_vertical(''ButtonMotion'');', ...
...
case 'trap'
    ...
    eid = 'Images:slice_marker_vertical:interruptedMouseSelection';
...
x1=p(1,1);
y1=p(1,2);
y1(y1<axes_Lim{2}(1))=axes_Lim{2}(1);%限定線段會在圖中
y1(y1>axes_Lim{2}(2))=axes_Lim{2}(2);%限定線段會在圖中
ydata = [y1 y1] ;
xdata = axes_Lim{1};
...
set(GETRECT_FIG,'WindowButtonUpFcn', 'slice_marker_vertical(''ButtonUp'');');            
