...
% These 2 are the HRF (1 trial length)
data_yavg       = DataClass().empty(); % Don't use the class (data matrix of # of HRF time points by # channels)
data_yavgstd    = DataClass().empty(); % same as above

nTrials         = []; % scalar
data_ynew       = DataClass().empty(); % Described above, # of raw time points by # of channels
data_ysum2      = DataClass().empty(); % Don't use this
data_yresid     = DataClass().empty(); % Might not use this
beta_blks       = cell(length(data_y),1); % Definitely need this
yR_blks         = cell(length(data_y),1); % Don't need
beta_label      = []; % Don't need
hmrstats        = []; % More advanced, want this eventually
...
for iBlk = 1:length(data_y) % This is a snirf thing, remove it (it was originally an array of structures)
    ...
    y      = data_y(iBlk).GetDataTimeSeries('reshape'); % already know
    t      = data_y(iBlk).GetTime(); % already know
    ml     = data_y(iBlk).GetMeasListSrcDetPairs('reshape'); % Don't need
    ...
    %%%%%%%%%%%%%%%%
    % stimStates allows us to turn off stims we don't want (don't need)
