...
ylabel('z pixels')
colorbar
vf.Position(4) = ceil(vf.Position(3) * hwRatio); % Set the aspect ratio
...
for filenum = 1:numFiles
    ...
    for f = 1:size(IQf, 4)
        ...
        %%%% Change this line to get different planes %%%%
        v1f = abs(IQf(40, :, :, f)); 
    
        imagesc(squeeze(v1f)')
        
        cv = getframe(vf);     % get the current frame
        rgb = frame2im(cv);      % convert the frame to rgb data
    
        writeVideo(vo, rgb); % write the frame to the video
    end

end

close(vo); % close the video object so it can be saved
