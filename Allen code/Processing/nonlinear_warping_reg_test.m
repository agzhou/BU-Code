...
y = y(:); x = x(:); z = z(:); % Make a meshgrid for evaluating the scatteredInterpolant at
tic
D = cpts.F(y(:), x(:), z(:)); % Calculate displacement field
toc
testD = reshape(D, ds(1), ds(2), ds(3), 3); % Reshape the deformation field
testD(isnan(testD)) = 0; % Remove NaNs
