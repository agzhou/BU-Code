...
clear bfi cai caiGlobal allCenters
%%
maxSpeedExpectedMMPerS = 100; % max expected flow speed [mm/s]
timePerFrame = 1 / P.frameRate;
maxDistPerFrameM = maxSpeedExpectedMMPerS * timePerFrame / 1000;
...
% for f = 3
    ...
    npC = size(currentFrame, 1); % number of points in the current frame
    npN = size(nextFrame, 1);    % number of points in the next frame
    D = NaN(npC, npN);

    if npC > 1 && npN > 1
        % Go through each current frame point and get the distance to each
        % point in the next frame. Store in the distance matrix D.
        for cpi = 1:npC % current point index
    %     for cpi = 1
            currentPoint = currentFrame(cpi, :);
            d = nextFrame - currentPoint;
            D(cpi, :) = sqrt(sum((d .^ 2), 2));
