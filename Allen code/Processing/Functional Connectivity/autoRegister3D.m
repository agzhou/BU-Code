%% Description
% - Registration for functional connectivity (in 3D space + time)
...
data_dirpath = uigetdir('F:\', 'Select the data directory');
...
%     [params_filename, params_pathname, ~] = uigetfile('*.mat', 'Select the params file', [data_dirpath, '..\params.mat']);
...
% 50 um version
[US_template_FilePathFN, US_template_FilePath] = uigetfile([data_dirpath, 'prereg_PDI_params_50um.mat'], 'Select the ultrasound template (50 um)');
US_template_FilePath = [US_template_FilePath, US_template_FilePathFN];
load(US_template_FilePath)
...
% 
% %% Store the manual registration transformation info
...
for bi = 1:size(PDIallBlocks, 4) % block index
% for sfi = 1:2
    disp(bi)
    PDI_bi = squeeze(PDIallBlocks(:, :, :, bi)); % PDI volume at superframe # sfi
    PDI_bi_rs = imresize3(PDI_bi, 'Scale', prereg_params.prereg_interp_factor, 'Method', 'cubic'); % Resize/resample

    if bi == 1
        PDIallBlocks_reg = zeros([size(AA_template_50um), size(PDIallBlocks, 4)], 'single'); % Initialize the registered PDI across time
    end
    % Apply the transformation
    PDIallBlocks_reg(:, :, :, bi) = single(imwarp(PDI_bi_rs, rigid_tform_50um, 'cubic', 'OutputView', Rout));
end
toc
...
save([data_dirpath, 'PDIallSF_reg_ROI_info_50um.mat'], "PDIallBlocks_reg", "roi", '-v7.3')
...
PDI_reg_global_mean = squeeze(mean(PDIallBlocks_reg, [1, 2, 3])); % Global mean timecourse (voxelwise) of the registered PDI
...
PDI_ROI_timecourses_mat = zeros(length(t), roi.num_regions); % Still ROI-averaged PDI timecourses, but in matrix form (each column is a separate ROI timecourse). Dimensions: [# time points, # ROIs]
for ri = 1:roi.num_regions % region/ROI index -- loop through each region
...
PDI_ROI_hemis_timecourses_mat = zeros(length(t), roi.num_regions*2); % Still ROI-averaged PDI timecourses, but in matrix form (each column is a separate ROI timecourse). Dimensions: [# time points, # ROIs]
for ri = 1:roi.num_regions % region/ROI index -- loop through each region
...
PDI_ROI_GMS_timecourses_mat = zeros(length(t), roi.num_regions); % Still ROI-averaged PDI timecourses, but in matrix form (each column is a separate ROI timecourse). Dimensions: [# time points, # ROIs]
for ri = 1:roi.num_regions % region/ROI index -- loop through each region
    PDI_ROI_GMS_timecourses_mat(:, ri) = PDI_ROI_GMS_timecourses{ri};
end

% Hemisphere-separated version
PDI_ROI_hemis_GMS_timecourses_mat = zeros(length(t), roi.num_regions*2); % Still ROI-averaged PDI timecourses, but in matrix form (each column is a separate ROI timecourse). Dimensions: [# time points, # ROIs]
for ri = 1:roi.num_regions % region/ROI index -- loop through each region
...
GVTD = squeeze( sum( diff(PDIallBlocks_reg, 1, length(size(PDIallBlocks_reg))) .^ 2, [1, 2, 3] ) ./ numVoxelsInVolume ) .^ 0.5;
...
generateTiffStack_acrossframes(PDIallBlocks_reg .^ 1, reg_volume_size_mm, 'hot', 1:size(PDIallBlocks_reg, 1), t)
