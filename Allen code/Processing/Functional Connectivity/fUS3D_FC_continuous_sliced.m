...
%   - ** Define some slices to use with sliceVolumeMotionTest.m **
...
%% Get the sagittal slicing info
% Load the slicing data
[slicingFilePathFN, slicingFilePath] = uigetfile([savepath, '\slicing_info.mat'], 'Select the slicing data');
slicingFilePath = [slicingFilePath, slicingFilePathFN];
load(slicingFilePath)

% Prompt for which slices to use
ssCropPrompt = {'Start slice', 'End slice'};
ssCropDefaults = {'', ''};
ssCropUserInput = inputdlg(ssCropPrompt, 'Input Parameters', 1, ssCropDefaults);

ss_start = str2double(ssCropUserInput{1});
ss_end = str2double(ssCropUserInput{2});
slicesToUse = ss_start:ss_end;
sagittal_range = [];
for ind = slicesToUse
    sagittal_range = [sagittal_range, csm{ind}]; % Indices in the sagittal direction to use
end
...
save([savepath, 'fUS_proc_params.mat'], 'sv_threshold_lower', 'sv_threshold_upper', 'zstart', 'zend', 'bs', 'bo', 'max_freq_expected', 'sagittal_range');
...
% for bn = 1
    ...
    % Crop the IQ first 
    IQm = IQ(sagittal_range, :, zstart:zend, :);
