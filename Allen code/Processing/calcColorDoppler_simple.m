...
function [CDI] = calcColorDoppler_simple(IQf_FT_separated, P)
...
%         %%%%%%%%%%%%%%%%% currently not thresholding %%%%%%%%%%%%%%%%%%%%%
        ...
        CDI = -fDi * P.Resource.Parameters.speedOfSound / 2 / (P.Trans.frequency * 1e6);
    end
...
% 
% % Output: CDI (same except Color Doppler index)
% function [CDI] = calcColorDoppler_simple(IQf, P)
% 
%     % Determine thresholds on the Fourier spectrum to get a more accurate
%     % Doppler frequency estimation
%     max_vz_expected = 0.05; % [m/s]
...
% 
%     IQsize = size(IQf);
%     CDI = zeros(IQsize(1), IQsize(2), IQsize(3));
% 
%     freqDim = length(size(IQf)); % Usually the frequency dimension is the last dimension. 3 for 2D data and 4 for 3D data.
% 
% 
%         Flim = [-P.frameRate/2, P.frameRate/2];
% 
%         fi = linspace(Flim(1), Flim(2), size(IQf, freqDim)); % frequencies
% 
% %         % Note: this line only works for 3D right now
%         if freqDim == 4
%             fi = repmat(permute(fi, [1, 4, 3, 2]), size(IQf, 1), size(IQf, 2), size(IQf, 3), 1);
%         elseif freqDim == 3
%             fi = repmat(permute(fi, [1, 3, 2]), size(IQf, 1), size(IQf, 2), 1);
%         end
% 
%             %         fDi = sum( abs(IQf_FT_separated{i} .^ 2) .* fi, freqDim) ./ sum( abs(IQf_FT_separated{i} .^ 2), freqDim); % Estimate the Doppler frequency
% %         fDi = sum( abs(IQf_FT_separated{i}) .^ 2 .* fi, freqDim) ./ sum( abs(IQf_FT_separated{i}) .^ 2, freqDim); % Estimate the Doppler frequency
