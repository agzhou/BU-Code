...
function [centersRC, refIQs, XC] = localizeBubbles3D_globalXCThreshold_subpixel_noparfor(IQf, refPSF, range, imgRefinementFactor, XCThreshold, zpixfactor)
    ...
    zrange = range{3};
    % framerange = range{4};
    % IQs = single(IQf(yrange, xrange, zrange, framerange)); % IQ section
    IQs = single(IQf(yrange, xrange, zrange, :)); % IQ section
...
%     IQs = IQf;
    ...
    if ~all(imgRefinementFactor == 1)
        refIQs = zeros(size(IQs, 1) * rfnX, size(IQs, 2) * rfnY, size(IQs, 3) * rfnZ, size(IQs, 4), 'single'); % refined IQ section
