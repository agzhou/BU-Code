...
function [g1] = g1T_3D(IQ_coherent_sum)
    tstart = clock;
    [xp, yp, zp, nf] = size(IQ_coherent_sum);

    g1 = zeros(xp, yp, zp, nf); % g1 is calculated for each voxel
                                % x, y, z, tau step
    numer = zeros(xp, yp, zp, nf);
    ...
    for f = 1:nf % go through each subframe to get the g1 at each tau step
        numer = mean(conj(IQ_coherent_sum(:, :, :, 1:(nf - f + 1))) .* IQ_coherent_sum(:, :, :, f:end), 4);
        g1(:, :, :, f) = numer ./ denom;
