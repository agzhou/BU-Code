...
function eyeROIs = defineEyeROIs(frame)
    ...
    eyeROIs.eye_mask = createMask(eye_ROI); % Create a mask from the eye ROI definition
    ...
    iris_intensity = imageData(round(irispos.Position(2)),round(irispos.Position(1)));
    
    % Use the mean between the pupil and iris points' intensities as a threshold for the transition
    eyeROIs.pupil_iris_th = (pupil_intensity + iris_intensity)/2;
    ...
    % 
    close(ff); % Close the figure when done
