%% Description:
%   TC (Transverse-Combined) analog of validate_vUS_3D_erf_Jac.m.
%   Validates vUS_3D_erf_TC_complex_Jac.m's analytic Jacobian against
%   central finite differences of the independently-written
%   vUS_3D_erf_TC_vec_split.m. Uses only base MATLAB -- no Symbolic Math
%   or Optimization Toolbox required, so it can run anywhere this
%   codebase runs, independent of whether the symbolic generator is
%   available.
%
%   Test points include a near-stagnant-flow case (small M) and use the
%   RC15gV matrix-probe PSF from test_vUS_3D_erf.m (sigma_x=sigma_y, as
%   this model requires), plus several step sizes per parameter (see
%   generate_vUS_2D_erf_Jac.m's header for why a single fixed step isn't
%   robust: k0*sigma sets an O(1-10) scale independent of flow speed, so
%   intermediate terms can be huge even when g1 itself is small).

sigma_test = [429; 429; 126] * 1e-6;       % [m], RC15gV probe, sigma_x=sigma_y
...
testPoints = { ...
    ...
    [ 1e-3;  15e-3; 0.3; 0.10], 'slow transverse, fast axial'};
% NOTE: 3e-4, not 1e-4 -- see generate_vUS_3D_erf_TC_Jac.m's header for
% why [1e-4,1e-4] is this probe's (RC15gV, sigma_z=126um) worst-
% conditioned corner specifically (a numerical-precision artifact
% confirmed there, not a formula error).
...
for i = 1:size(testPoints, 1)
    ...
    [~, J_analytic] = vUS_3D_erf_TC_vec_split_Jac(x0, tau_test, k0_test, sigma_test);
    ...
    for h = hList
        for p = 1:4
            ...
            % entries near zero (e.g. Im(dg1/dv_tgp) as tau->0, since
