...
% Outputs:
%   vUS: [x voxels, y voxels, z voxels, 2 (transverse and axial components??)] real velocity data matrix
...
% sv_threshold_lower = 20; sv_threshold_upper = size(IQ, fDim);

% Mask the region to actually process
[xpo, ypo, zpo, nfo] = size(IQ); % Original sizes
...
tempPDI = sum(abs(IQf).^2, fDim)./noise;
figure; imagesc(squeeze(max(tempPDI, [], 1))' .^ 0.5)
