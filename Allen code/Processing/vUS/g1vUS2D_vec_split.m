%% Description:
%   Calculate the g1 model as in the vUS paper (Eq. 15 in Jianbo Tang et al., 2020)
%   This is just a version where parameters are a vector input, which is
%   what fitting functions require
%
%   2D analog of g1vUS3D_vec_split.m, using the full complex model (real
%   cos/imag sin split) rather than g1vUS3D_vec_split.m's current
%   magnitude-only-with-zero-imaginary-part fallback -- this is meant for
%   fitting the actual complex g1(tau), not just |g1(tau)|.

% Inputs:
%   x: v_xgp, v_zgp, p, optional: [F, DC component] (all in SI units)
%   tau: time lag vector [any units, probably seconds]
%   sigma: 1/e values of the PSF shape in x, z [m]
%   k0: Wavenumber [rad/m]
%   useF: boolean value --> consider the F parameter or not
%   useDC: boolean value --> consider the DC parameter (static component) or not

% Outputs:
%   g1_split: g1(tau) split into its real and imaginary components, shape [nTau, 2]

function [g1_split] = g1vUS2D_vec_split(x, tau, sigma, k0, useF, useDC)

    v_xgp = x(1);
    v_zgp = x(2);
    p = x(3);
    if useF
        F = x(4);
    else
        F = 1; % No need for F if we start at tau = 0 --> |g1(0)| = 1
    end

    if useDC
        DC = x(5);
    else
        DC = 0;
    end

    envelope = exp(-(v_xgp .* tau).^2 ./ (4 * sigma(1)^2) ...
        - (v_zgp .* tau).^2 ./ (4 * sigma(2)^2)) ...
        .* exp(-(p .* v_zgp .* k0 .* tau).^2);
    phase = 2 .* k0 .* tau .* v_zgp;

    g1_real = DC + F .* envelope .* cos(phase);
    g1_imag = F .* envelope .* sin(phase);

    g1_split = [g1_real(:), g1_imag(:)];
end
