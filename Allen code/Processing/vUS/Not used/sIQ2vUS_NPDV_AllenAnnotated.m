...
function [Mf, Vz, V, pVz, Vcz, R, CR, Vx, Ms, pnRatio,GGf,sumGG0,sumGGV0,sumGGFV0, GGfid]=sIQ2vUS_NPDV(sIQ, PRSSinfo)
%% O. constant
...
%     clear iGG
    % V.3. g1(1) adjust (======== AZ: Screen pixels for good ones ========)
...
%     MfR0 = max(min(1-Ms00-Me0,real(GG2(:,1))),0);
    ...
    [g1Vz0, Tvz]=GG2Vz(GG2, PRSSinfo, 10); % (======== AZ: Find tau_V as in Eq. 16 ========) %
    [iVz, iVx, ipVz, iMs, iMf, iR, iGGf, iGGfid]=GG2vUS_fit(GG2, reshape(g1Vz0,[nz0*nx0,1]), Ms00, MfR0, PRSSinfo);
...
%     [iVz, iVx, ipVz, iMs, iMf, iR, iGGf]=GG2vUS(GG2, reshape(iVcz,[nz0*nx0,1]), Ms00, MfR0, PRSSinfo);
    Ms0(:,:,iNP)=(iMs); Mf0(:,:,iNP,:)=(iMf(:,:,:));
    Vx0(:,:,iNP)=(iVx).*iCR; Vz0(:,:,iNP)=(iVz).*iCR; Vcz0(:,:,iNP)=-1*iVcz.*iCR;
    pVz0(:,:,iNP)=(ipVz).*iCR; R0(:,:,iNP)=(iR); GGf0(:,:,:,iNP)=(iGGf);GGfid(:,:,:,iNP)=(iGGfid);
    CR0(:,:,iNP)=(iCR);
    pnRatio0(:,:,iNP)=(ipnRatio);
    %% G1-fUS calculation 01092022 Bingxue Liu
    GG2_ = smoothdata(abs(GG2), 2, 'sgolay', 11);
%     iGG__ = reshape(GG2_, [PRSSinfo.Dim(1),PRSSinfo.Dim(2),PRSSinfo.Dim(3)]);
%     iGG_ = reshape(GG2, [PRSSinfo.Dim(1),PRSSinfo.Dim(2),PRSSinfo.Dim(3)]);
%     figure(1);subplot(311);plot(movmean(abs(squeeze(iGG_(slt(1),slt(2),:,1))),1));%ylim([0,0.6]);
%     hold on; plot((abs(squeeze(iGG__(slt(1),slt(2),:,1)))));
%     subplot(312);plot(movmean(real(squeeze(iGG_(slt(1),slt(2),:,1))),1));
%     hold on; plot((real(squeeze(iGG__(slt(1),slt(2),:,1)))));
%     subplot(313);plot(movmean(imag(squeeze(iGG_(slt(1),slt(2),:,1))),1));
%     hold on; plot((imag(squeeze(iGG__(slt(1),slt(2),:,1)))));
    sumGG00(:,iNP) = squeeze(sqrt(abs(-log((GG2_(:,5))./(GG2_(:,1))))));
    sumGGV00(:,iNP) = squeeze((GG2_(:,1))./(1-(GG2_(:,1))));
    sumGGFV00(:,iNP) = squeeze((real(GG2(:,1))-(real(GG2(:,10)))));

end
    GG0 = sIQ2GG(sIQ, PRSSinfo);
    GG0 = reshape(GG0, [PRSSinfo.Dim(1)*PRSSinfo.Dim(2),PRSSinfo.Dim(3)]);
    sumGG00(:,3) = squeeze(sqrt(abs(-log(abs(GG0(:,5))./abs(GG0(:,1))))));
    sumGGV00(:,3) = squeeze(abs(GG0(:,1))./(1-abs(GG0(:,1))));
    sumGGFV00(:,3) = squeeze((real(GG0(:,1))-(real(GG0(:,10)))));
   sumGG0 = reshape(sumGG00, [PRSSinfo.Dim(1),PRSSinfo.Dim(2),3]);
   sumGGV0 = reshape(sumGGV00, [PRSSinfo.Dim(1),PRSSinfo.Dim(2),3]);
   sumGGFV0 = reshape(sumGGFV00, [PRSSinfo.Dim(1),PRSSinfo.Dim(2),3]);
Vz0(:,:,1)=-1*abs(Vz0(:,:,1));
Vz0(:,:,2)=1*abs(Vz0(:,:,2));
% Vx0(abs(Vx0)>40)=20;
% III. 3 MaskCR
if PRSSinfo.rfnScale>1
    ...
    GGfid(:,:,:,3)=GGfid(:,:,:,1).*repmat(pnRatio(:,:,1),[1,1,PRSSinfo.g1nTau])+GGfid(:,:,:,2).*repmat(pnRatio(:,:,2),[1,1,PRSSinfo.g1nTau]);
else
