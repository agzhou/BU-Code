...
for j = 1
    ...
    for vi = ind
        ...
        % if vesselAngleMask(vi) % Fit only voxels we believe have high signal quality
            ...
            % x0 = [tauC0(vi), FR_j(vi), abs(DCR0_j(vi))];
            x0 = [1e3, FR_j(vi), abs(DCR0_j(vi))];
            ...
            lb = [0, max(FR_j(vi) - 0.2, 0), max(DCR0_j(vi) - 0.2, 0)]; % TESTING
            ub = [1e3, min(FR_j(vi) + 0.2, 1), min(DCR0_j(vi) + 0.2, 1)]; % TESTING

            mag_g1_exp_j_vi = abs(g1_exp_j(vi, :)); mag_g1_exp_j_vi = mag_g1_exp_j_vi(:);
            anon_fun = @(x) g1ModelSimplified2D_vec(x, tau, PP.k0, useF, useDC) - mag_g1_exp_j_vi;
