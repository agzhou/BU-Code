...
% anglesRXList = anglesTXList;
anglesRXList = anglesRXList(:);
anglesRX = listToAnglesRCA(anglesRXList, 'RX');

% Test Configuration 1: Almost Nikunj's offset scheme
% nikunjOffset = daTX/2;
% anglesRX(1:naRX, 2) = anglesRXList + nikunjOffset;
% anglesRX(naRX + 1:2*naRX, 1) = anglesRXList + nikunjOffset;

% Test Configuration 2: Nikunj's full offset scheme
offset = daTX;
 % test = mean(diff(anglesTX, 1, 1), 1)
anglesRX = flip(anglesTX, 2);
anglesRX(1:naRX, 2) = anglesRX(1:naRX, 2) + offset/2;
anglesRX(naRX + 1:end, 1) = anglesRX(naRX + 1:end, 1) + offset/2;
...
imagesc(xCoord*1e3, zCoord*1e3, squeeze(max(abs(ReconKK), [], 2))' .^ 0.5); colormap gray
title('KK')
xlabel('x [mm]')
ylabel('z [mm]')
% KKMIP_fh.Position(4) = KKMIP_fh.Position(3)*( max(zCoord) - min(zCoord) )/( max(xCoord) - min(xCoord) );
