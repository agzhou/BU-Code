...
% **** NOTE: THIS BEAMFORMS ONLY A SELECTED SUBSET OF ANGLES ****
% Inputs:
...
function [BFData, varargout] = BeamformKK_RCA_test(RawDataKK, anglesTX, anglesRX, anglesTXKept, anglesRXKept, BFgrid, param, interp_method, varargin)
    ...
    naTXKept = size(anglesTXKept, 1); % Total # of TX angles kept
    naRX = size(RawDataKK, 3); % # of RX angles
    naRXKept = size(anglesRXKept, 1); % Total # of RX angles kept

    n = naTXKept/2; % Define a variable corresponding to the number of TX angles for one shot (CR or RC), which should be the same as the # of RX angles
    if naTXKept ~= naRXKept
        error('this scheme only supports an equal number of TX and RX angles across CR and RC acquisitions')
    end
    [nx, ny, nz] = size(BFgrid.X); % Get the size of the grid

    % **** BREAK UP ANGLES INTO RC AND CR **** %
    indsRX_CR = 1:naRXKept/2;
    indsRX_RC = naRXKept/2 + 1:naRXKept;
    anglesRX_CR = anglesRXKept(indsRX_CR, 2);
    anglesRX_RC = anglesRXKept(indsRX_RC, 1);

    indsTX_CR = 1:naTXKept/2;
    indsTX_RC = naTXKept/2 + 1:naTXKept;
    anglesTX_CR = anglesTXKept(indsTX_CR, 1);
    anglesTX_RC = anglesTXKept(indsTX_RC, 2);
    ...
    switch whatToReturn
        ...
        case 'compounded'
            ...
            % C-R
            tempCR = zeros(nx, ny, nz); % Initialize a volume to keep adding to
            for j = 1:n
                disp(j)
                ...
                % tempAngleTX = [anglesTX_CR(j), 0];
                % tempAngleRX = [0, anglesRX_CR(j)]; % Could also do anglesRXKept(j, :) and j + n for RC
                
                % FLIPPING HERE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! In the
                % RawDataKK access as well
                tempAngleTX = anglesRXKept(j, :);
                tempAngleRX = anglesTXKept(j, :);
                tempLUTTX = genLUT(tempAngleTX, BFgrid, param.c);
                % tempLUTRX = squeeze(LUTRX(:, :, :, rai)); % Temporarily store the RX time delays for angle index rai
                tempLUTRX = genLUT(tempAngleRX, BFgrid, param.c);

                [verytemp] = applyTimeDelays(nx, ny, nz, ns, tempLUTTX, tempLUTRX, param, squeeze(RawDataKK(:, find(anglesTX(1:naTX/2, 1) == tempAngleRX(1)), find(anglesRX(1:naRX/2, 2) == tempAngleTX(2)))), interp_method);
                tempCR = tempCR + verytemp;
                
            end
            BFData = BFData + tempCR;
            
            tempRC = zeros(nx, ny, nz); % Initialize a volume to keep adding to
            for j = 1:n
                disp(n + j)
                tempAngleTX = anglesRXKept(n + j, :);
                tempAngleRX = anglesTXKept(n + j, :);
                tempLUTTX = genLUT(tempAngleTX, BFgrid, param.c);
                tempLUTRX = genLUT(tempAngleRX, BFgrid, param.c);

                [verytemp] = applyTimeDelays(nx, ny, nz, ns, tempLUTTX, tempLUTRX, param, squeeze(RawDataKK(:, naTX/2 + find(anglesTX(naTX/2 + 1:naTX, 2) == tempAngleTX(2)), naRX/2 + find(anglesRX(naRX/2 + 1:naRX, 1) == tempAngleRX(1)))), interp_method);
                tempRC = tempRC + verytemp;
                
            end
            BFData = BFData + tempRC;
