...
function [IQ] = RCA_DAS_fix(RcvData, P, zpix_spacing)
    ...
    tw_Numpulses = TW.Numpulses;
    
    elemSens = Trans.ElementSens;
...
%     parfeval(@assignStructVars, 0, P); % numel(fieldnames(P))
    ...
    % Take the attenuation value from the Media structure
...
%     nzs_nopad = s(1) / na / 2;
    ...
    % (# samples, # elements, # acquisitions (2 * # angles), # frames)
    d = permute(d, [2, 1, 3, 4]); 
...
%     d_sum = zeros(xnumpix, ynumpix, znumpix, na*2, nf, 'single'); %%%%%%%%%%%%%%
    d_sum = zeros(xnumpix, ynumpix, znumpix, na*2, nf); %%%%%%%%%%%%%%
...
%             for xp = 1:xnumpix
            for xp = 60
                ...
                % rcv distance from pixel to elements is same across all
                % transmissions in one angular direction
...
%                 temp_td = traveldistTX_temp + traveldistRX_temp + wl* (tw_peak); % TW.peak is the # of wavelengths to peak of transmitted waveform
                temp_td = traveldistTX_temp + traveldistRX_temp + wl* (1 + tw_peak); % TW.peak is the # of wavelengths to peak of transmitted waveform
...
%                     end
%                 end
                dd = reshape(d_delayed_temp, [numElements, na*2, nf]);
    ...
    %             element_sens_p_y = interp1(linspace(-pi/2, pi/2, length(elemSens)), elemSens, angle_p_y);
    %             element_sens_p_x = interp1(linspace(-pi/2, pi/2, length(elemSens)), elemSens, angle_p_x);
    ...
    %             dd([repmat(~element_sens_mask_y', 1, na, nf), repmat(~element_sens_mask_x', 1, na, nf)]) = 0;
                ...
                d_sum(xp, yp, zp, :, :) = squeeze(sum(dd, 1)); % sum over elements
            end
        end
