...
% change 9/12/24
nspa = 4*(2*(maxAcqLength - startDepth)); % see p104. Receive does 2x of (maxAcqLength - startDepth)
nspa = 128 * ceil(nspa/128); % # samples per acquisition
maxAcqLength_adjusted = nspa / 4 / 2;

Receive = repmat(struct('Apod', ones(1, Trans.numelements), ...
                        'startDepth', startDepth, ...
                        'endDepth', maxAcqLength_adjusted + startDepth, ... % change 9/12/24, was previously just maxAcqLength
                        'TGC', 1, ...
                        'bufnum', 1, ...
                        'framenum', 1, ... 
                        'acqNum', 1, ...
...
% Resource.RcvBuffer(1).rowsPerFrame = na*(maxAcqLength - startDepth)*8; % 
% Resource.RcvBuffer(1).rowsPerFrame = na*256*8; % 
                                              % from other code with 4096: this size allows for maximum range 
                                              % from tutorial: 2048 rowsPerFrame allows for max depth of rowsPerFrame/4 = 512 wavelengths. Then divide by 2 bc acquisition length is the round trip distance = 256. With the 4x sampling rate, there are 4 samples per wavelength. rowsPerFrame is in units of samples.


% nspa = 4*ceil(maxAcqLength + (endDepth - startDepth));
% nspa = 128 * ceil(nspa/128); % # samples per acquisition



Resource.RcvBuffer(1).rowsPerFrame = nspa * numSubFrames*na;
...
% manual p105
acqsPerFrame = [1:200]; % range to graph for
...
n = Resource.RcvBuffer(1).rowsPerFrame./na.*acqsPerFrame;
% n = mod(n, 128) + n;
n = 128 * ceil(n/128); % round up to nearest multiple of 128, since VSX transfers in chunks of 128 samples

% number of z samples per element * # elements * 2 bytes (16 bits) per
% int16 * 1024^2 bytes per megabyte
RcvDataSize = n * Resource.RcvBuffer.colsPerFrame * 2 / 1024^2; % in MB
...
% save('MatFiles/setup_L22_14v_ctsacq1');
